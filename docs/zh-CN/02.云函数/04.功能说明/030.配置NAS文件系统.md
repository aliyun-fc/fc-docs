...
3. 在函数详情页面，选择**配置**页签，单击**高级配置**右侧的**编辑**，在**高级配置**面板，找到**存储**选项，开启**挂载NAS文件系统**开关，按照以下操作配置完成后单击**部署**。
  ...
  | **用户**、**用户组** | 需填写自定义的用户ID和用户组ID。如您不输入任何值，系统将使用root用户，即`UID=0`和`GID=0`。更多信息，请参见[NAS用户和用户组](#nas_4)。 | 1 |
  | **远端目录** | - 通用型NAS的该目录必须以/开头。<br>- 极速型NAS的该目录必须以/share开头。<br>若您配置的目录在远端NAS中不存在，函数计算将会为您自动创建该目录。目录所有者为上述配置的用户和用户组，权限等级为`777`。<br>更多信息，请参见[远端目录](#远端目录和函数本地目录)。 | / |
  | **函数本地目录** | 建议使用/home、/mnt、/tmp或/data的子目录。<br>**<br>**说明**<br>不能使用通用的Linux和Unix系统目录及其子目录，例如/bin、/opt、/var或/dev等，以免挂载失败。<br>更多信息，请参见[函数本地目录](#远端目录和函数本地目录)。 | /mnt/nas |
