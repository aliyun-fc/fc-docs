...
由于函数计算的运行环境是Linux系统，您在Windows系统或macOS系统安装emoji依赖库时如果带有二进制文件，会导致您的代码包上传到函数计算后运行失败。因此，建议您[使用WebIDE打包函数第三方依赖](https://help.aliyun.com/zh/functioncompute/fc/developer-reference/use-webide-to-package-third-party-dependencies-of-a-function-1)或者[使用Serverless Devs工具安装依赖并部署项目](#serverless-devs)。
