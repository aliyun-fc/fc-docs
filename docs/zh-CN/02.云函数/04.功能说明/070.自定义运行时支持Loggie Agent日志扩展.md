...
1. 函数创建成功后，在**代码**页签使用WebIDE在`code`目录下创建`bootstrap`文件。
  ...
  1. 创建配置文件pipelines.yml，pipelines.yml为Pipeline配置文件。
    ...
    - `sink`
      ...
      用于指定日志服务相关信息。关于参数的说明，请参见[配置参数说明](https://help.aliyun.com/zh/sls/use-loggie-to-upload-logs#section-8rn-cgw-n0b)。脚本中的变量会在[步骤四](#步骤四验证结果)设置。
