...
## 为函数配置会话实例隔离 {#为函数配置会话实例隔离}

### **步骤一：会话实例隔离配置**

您可以**创建新函数**并启用会话隔离，也可为**已有函数**配置该功能。若对现有函数进行配置，请确保其满足[适用范围](#适用范围)中的实例规格、运行时及函数类型等要求。
...
1. 登录[函数计算控制台](https://fc.console.aliyun.com/)，在页面上方切换地域，如：**华东1（杭州）**；
  ...
  建议在部署前查看[适用范围](#适用范围)，选择支持相关功能的地域。
...
1. 登录[函数计算控制台](https://fc.console.aliyun.com/)，在页面上方切换目标函数所在地域，如：**华东1（杭州）**；
  ...
  建议在部署前查看[适用范围](#适用范围)，选择的函数所在地域需要支持相关功能。
...
2. 参考[为函数配置会话实例隔离](#为函数配置会话实例隔离)步骤，开启**会话实例隔离**配置；
...
- `Header Key`的值与[为函数配置会话实例隔离](#为函数配置会话实例隔离)设置的值保持一致（如：`x-affinity-header-v1`），实现会话绑定路由。
...
![image](../../../../assets/zh-CN/3001390-001-5c22d5a92d.png)
