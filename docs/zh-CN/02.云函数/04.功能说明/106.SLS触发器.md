...
2. 在**配置测试参数**面板，选择**创建新测试事件**或**编辑已有测试事件**，填写事件名称和事件内容，然后单击确定。如果是创建新测试事件，建议事件模板选择**日志服务**。测试数据的具体配置请参见[event](#event)。
3. 虚拟事件配置完成后，单击**测试函数**。
...
- ### **denied by sts or ram, action: log:GetCursorOrData, resource: ******
  ...
  如果函数日志中出现以上错误，可能是因为函数未配置权限，或权限策略设置不正确引起。请参见[步骤二：配置权限](#步骤二配置权限)。
