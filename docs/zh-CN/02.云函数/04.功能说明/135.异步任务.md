...
您可以通过调用[调用函数](https://help.aliyun.com/zh/functioncompute/fc/developer-reference/api-fc-2023-03-30-invokefunction)接口，x-fc-invocation-type参数选择Async，触发一次异步调用来启动任务模式。您可以在调用时添加HTTP请求头x-fc-async-task-id来为本次调用设置任务ID，更多信息，请参见[任务防重复提交](#任务防重复提交)。
