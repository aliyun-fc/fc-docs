...
首先，函数的`handler`需要包含`context`参数，函数计算会把[Context信息](#context)中的变量信息插入到`context`的取值中。然后，需要`import aliyun/fc-runtime-go-sdk/fccontext`，通过`fccontext.FromContext`方法获取`fccontext`。
