...
函数实例生命周期回调程序与正常调用请求计费规则一致，但其执行日志只能在**函数日志**或**高级日志**中查询，**调用请求**不会展示回调程序日志。具体操作，请参见[查看实例生命周期回调函数日志](#查看实例生命周期回调函数日志)。
...
| **Path** | **输入请求** | **期望的响应** |
| --- | --- | --- |
| （可选）POST`/initialize` | 请求体：无。<br>请求头：Common Request Headers。具体信息，请参见[函数计算公共请求头](https://help.aliyun.com/zh/functioncompute/fc/user-guide/context-and-log-format-1#section-3f8-5y1-i77)。 | 响应体：函数`Initializer`的返回值。<br>StatusCode<br>- 2xx：成功状态。<br>- 非2xx：失败状态。<br>**<br>**说明**<br>当Initializer回调程序执行超时或失败时，服务端始终返回HTTP 200状态码，必须通过响应头`X-Fc-Error-Type:InitializationError`或响应体中的errorMessage字段判断是否因初始化失败导致错误。 |
...
这里以自定义运行时Python 3.10为例，为您介绍自定义运行时生命周期回调方法，具体示例程序如下所示。
...
如果您想在自定义运行时中使用Initializer回调方法，您只需在您的HTTP Server中实现Path为`/initialize`和Method为`POST`的对应逻辑即可。示例代码，请参见上表中关于`initialize`的示例代码。

**

**重要**

如果创建的函数不设置Initializer，就无需实现`/initialize`。此时，即使HTTP Server实现了`/initialize`，代码中的`/initialize`逻辑也无法被调用执行。
...
3. 在函数详情页面，选择**配置**页签，单击**实例配置**区域右侧的**编辑**。
4. 在**实例配置**面板，设置回调程序和回调超时时间，然后单击**部署**。
5. 配置回调函数之后，您需要在代码执行中实现对应的函数，单击代码编辑器上方的**部署代码**，然后单击**测试函数**。
