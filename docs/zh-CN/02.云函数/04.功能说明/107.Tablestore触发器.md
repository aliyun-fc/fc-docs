...
![image](../../../../assets/zh-CN/2513640-001-50347db77c.svg)
...
4. 在**实例详情**页签的**数据表列表**页签，单击数据表名称后选择**实时消费通道**页签或单击![fig_001](../../../../assets/zh-CN/2513640-002-778c87b7c3.png)后选择**实时消费通道**。
...
1. 在函数详情页面的**代码**页签，单击**测试函数**右侧的![image.png](../../../../assets/zh-CN/2513640-003-51d4269d35.png)图标，从下拉列表中，选择**配置测试参数**。
...
- 如果您无法在某一地域创建Tablestore触发器，请确认支持创建Tablestore触发器的地域，具体请查看[使用限制](#使用限制)。
...
- 如果Tablestore数据表中有新增的数据，但是Tablestore触发器没有被触发，您可以从以下方面进行排查。关于触发器不能正常触发的详细排查方案可参见[触发器不能正常触发函数执行怎么办？](https://help.aliyun.com/zh/functioncompute/fc/what-to-do-if-a-trigger-cannot-trigger-function-execution)。
  ...
  - 确认数据表是否开启了Stream功能，具体请参见[步骤一：为数据表开启Stream功能](#stream)。
  - 确认在创建触发器时配置的角色是否正确，您可以使用默认的触发器角色`AliyunTableStoreStreamNotificationRole`，具体请参见[步骤二：创建Tablestore触发器](#tablestore_1)。
