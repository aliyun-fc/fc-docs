...
1. [HTTP Server连接主动关闭](#http-server)。
2. [函数本身原因导致进程退出](#函数本身的原因导致进程退出)。
3. [实例内存不足导致进程OOM](#oom)。
4. [启动命令缺少可执行权限、文件不存在、文件格式错误](#启动命令缺少可执行权限文件不存在文件格式错误)。
...
## **启动命令缺少可执行权限、文件不存在、文件格式错误** {#启动命令缺少可执行权限文件不存在文件格式错误}
