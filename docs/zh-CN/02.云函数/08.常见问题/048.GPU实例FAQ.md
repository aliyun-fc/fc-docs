...
- [函数计算GPU实例的驱动和CUDA版本是什么?](#gpucuda)
- [执行时遇到CUFFT_INTERNAL_ERROR怎么办？](#cufft_internal_error)
- [构建镜像时报错CUDA GPG Error如何解决？](#cuda-gpg-error)
- [为什么我的GPU实例规格显示的是g1？](#gpug1)
- [为什么实例启动失败？](#为什么实例启动失败)
- [无法正常弹出GPU弹性实例，提示"ResourceExhausted"、"ResourceThrottled"如何处理？](#gpuresourceexhaustedresourcethrottled)
- [GPU镜像大小限制是多少？](#gpu)
- [GPU镜像加速转换失败怎么办？](#gpu_1)
- [模型应该打在镜像里，还是与镜像分离？](#模型应该打在镜像里还是与镜像分离)
- [如何做模型预热，有没有最佳实践？](#如何做模型预热有没有最佳实践)
- [GPU镜像启动报错“FunctionNotStarted”Function Instance health check failed on port xxx in 120 seconds怎么办？](#gpufunctionnotstartedfunction-instance-health-check-failed-on-port-xxx-in-120-seconds)
- [我的函数端到端延迟较大，并且波动很大，需要怎么处理？](#我的函数端到端延迟较大并且波动很大需要怎么处理)
- [无法找到NVIDIA驱动程序怎么办？](#nvidia)

## 函数计算GPU实例的驱动和CUDA版本是什么?

GPU实例涉及的组件版本主要分为以下两个部分：

- **驱动版本**：包含内核态驱动`nvidia.ko`、CUDA用户态驱动`libcuda.so`等。函数计算GPU实例所使用的驱动由NVIDIA提供，由函数计算平台负责部署。随着功能迭代、新卡型推出、BUG修复、驱动生命周期到期等原因，GPU实例所使用的驱动版本未来可能变化，请避免在容器镜像中添加驱动特定相关内容，更多内容，请参见[无法找到NVIDIA驱动程序怎么办？](#nvidia)。
