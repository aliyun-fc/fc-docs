...
- 不要在源镜像中自行设置 `ENTRYPOINT ["/.fce2b/entrypoint"]` 或复制 `/.fce2b`。使用普通业务镜像时，`Template.build()` 的 builder 模式会注入这些运行时文件并设置入口；预先声明一个尚不存在的入口会导致 `envd inject failed`。完整镜像要求、ACR EE 网络和 builder 配置见[构建自定义镜像模板](../02.模板/03.构建自定义镜像模板.md)。
