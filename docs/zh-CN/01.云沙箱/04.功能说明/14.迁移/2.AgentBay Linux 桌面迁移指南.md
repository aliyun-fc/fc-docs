...
| AgentBay | FC E2B | 说明 |
|--------|------|------|
| `linux_latest`（Computer Use） | Desktop 模板（镜像 `fc-e2b-registry.cn-beijing.cr.aliyuncs.com/runtime/desktop:v0.0.52`） | `v0.0.52` 是当前推荐的默认版本，通过 `Template.build()` 构建，见 §4 |

## 4. 前置准备：Desktop 模板

Desktop 模板不是开通即用的内置模板。推荐直接使用 FC 已发布的 Desktop 镜像制作 Template；只有需要预装额外软件、修改桌面环境或固化企业配置时，才需要自行构建镜像。

### 4.1 推荐：使用 FC Desktop `v0.0.52` 镜像

FC Desktop `v0.0.52` 已包含桌面环境、Chrome、Xvfb、x11vnc、noVNC/websockify，以及 `e2b-desktop` 所需的兼容入口。
...
- 不要使用系统保留前缀作为 Template 名称。保留前缀包括：`base`/`base-v*`、`desktop`/`desktop-v*`、`code-interpreter-v*`、`claude`/`claude-v*`、`codex`/`codex-v*`、`openclaw`/`openclaw-v*`、`opencode`/`opencode-v*`、`amp`/`amp-v*`；例如使用 `custom-desktop-v0052-20260916`。
...
# 公网受限模式必须由后端代理注入 e2b-traffic-access-token 请求头，query 参数无效。
...
```
- 浏览器直连 `wss://{sandbox-host}/vnc.html` 无需自定义请求头（实测 GET 200、RFB 握手成功）；WebSocket 握手不会附加 `Content-Disposition`，但默认平台域名（`*.e2b.fc.aliyuncs.com`）下用浏览器直接打开该地址会**触发下载而非渲染页面**，需先[绑定自定义域名](../07.FC%20扩展/03.自定义域名.md)；VNC 鉴权由 `stream.start(require_auth=True)` + `get_url(auth_key=...)` 控制，开启后把 `auth_key` 一并返回前端并传入 `credentials.password`。
- 公网受限模式（`allow_public_traffic=False`）必须使用 `e2b-traffic-access-token` 请求头，query 参数无效。运行在浏览器页面内的 WebSocket 客户端无法自定义该请求头，应通过受控后端代理连接；服务端客户端可直接在握手头携带该令牌；VNC 的 `password` 参数只用于 VNC 自身鉴权。
...
def ensure_template() -> str:
    """确保桌面模板存在，复用前校验名称中的镜像版本标记。

    SDK 的 Template.exists() 仅返回 bool、无法查询已建模板的构建镜像，
    因此在模板名中编码镜像版本+日期作为防复用校验机制：
    名称含期望版本标记时复用，否则改用带版本+日期的新名字重建。
    """
    expected_tag = _extract_image_version_tag(IMAGE)
    if Template.exists(TEMPLATE):
        if expected_tag and expected_tag not in TEMPLATE:
            # 名称不含期望版本标记，说明是旧镜像构建的模板；
            # 改用带版本+日期的新名字重建，避免静默复用旧镜像。
            new_name = f"custom-desktop-{expected_tag}-{time.strftime('%Y%m%d')}"
            if Template.exists(new_name):
                # 同日已按该名字重建过，直接复用，避免重复 build 撞 409；
                # 请把新名字写回 .env 的 E2B_DESKTOP_TEMPLATE。
                print(f"[demo] 复用同日重建的模板: {new_name}（建议写回 .env）", flush=True)
                return new_name
            print(f"[demo] 模板 {TEMPLATE!r} 存在但名称不含 {expected_tag}，"
                  f"改用新名字 {new_name!r} 重建", flush=True)
...
def _extract_image_version_tag(image: str) -> str:
    """从镜像地址提取版本标记，如 desktop:v0.0.52 → v0052；digest 或无 tag 返回空。"""
...
def _build_template(name: str) -> str:
    ...
    if not IMAGE:
        raise SystemExit(
            f"[demo] 模板 {name!r} 不存在，请在 .env 配置 E2B_DESKTOP_IMAGE 以自动构建"
        )
    print(f"[demo] 从镜像构建模板 {name!r}: {IMAGE}", flush=True)
    build = Template.build(
        Template().from_image(IMAGE),
        name=name,
        cpu_count=4,
  ...
  <script type="module">
    ...
    async function initVNC() {
      ...
      const rfb = new RFB(
        ...
        // 作为 WebSocket 端点交给 RFB（握手不附加 Content-Disposition）；
        // 默认平台域名下用浏览器直接打开该地址会触发下载，需绑定自定义域名
        streamUrl,
        { credentials: { password: authKey } }
      );
