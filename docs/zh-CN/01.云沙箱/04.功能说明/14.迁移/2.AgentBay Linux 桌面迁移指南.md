...
- 不要使用 `desktop-v*` 作为 Template 名称前缀，该前缀为系统保留；例如使用 `custom-desktop-v0052`。
...
- 公网受限模式（`allow_public_traffic=False`）必须使用 `e2b-traffic-access-token` 请求头，query 参数无效。浏览器 WebSocket 无法自定义该请求头，应通过受控后端代理连接；VNC 的 `password` 参数只用于 VNC 自身鉴权。
