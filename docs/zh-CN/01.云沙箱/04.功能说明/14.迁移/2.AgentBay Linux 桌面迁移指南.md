...
以下截图来自基于该 Ubuntu + GNOME `start-desktop` 示例新建的 Sandbox。验证脚本未模拟键盘输入；在完整桌面上启动镜像自带的 X11 应用 `xmessage` 后截图，确认桌面和应用窗口均正常显示，而非 Activities 概览：

![新建 Sandbox 显示完整 GNOME 桌面和 xmessage 应用窗口](../../../../../assets/zh-CN/agentbay-gnome-default-desktop.png)
