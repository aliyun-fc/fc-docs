...
如果应用依赖 E2B Volume、MCP gateway、BYOC、网络高级配置、高频 Metrics，请先改造设计，不要直接迁移。Snapshot 已支持，但命名和恢复约束与官方 E2B 不完全相同，参见[快照](../01.沙箱/12.快照.md)。云沙箱支持查看 Sandbox 指标，当前按 1 分钟粒度返回。
