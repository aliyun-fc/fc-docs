...
执行结果 `Execution` 包含 `logs`（stdout/stderr 列表）、`results`（富文本结果，含 `text`、`png` 等）、`error`（执行异常）和执行计数（Python 为 `execution_count`，TypeScript 为 `executionCount`）。
...
- [browser 模板](../09.示例/01.browser-模板.md)（仅需浏览器自动化能力时选择）
- [All-In-One 模板](../09.示例/02.all-in-one-模板.md)（需要同时使用浏览器和代码执行能力时选择）
