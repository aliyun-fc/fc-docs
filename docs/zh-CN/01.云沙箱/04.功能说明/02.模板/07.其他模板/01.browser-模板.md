...
| **配置项** | **默认值** | **说明** |
| --- | --- | --- |
| 容器镜像 | `fc-e2b-registry.cn-beijing.cr.aliyuncs.com/runtime/browser:v0.0.39` | 预置 browser 镜像 |
| 默认端口 | 3000 | 沙箱服务监听端口 |
| CPU | 4 vCPU | 推荐起始规格 |
| 内存 | 8192 MB | 推荐起始规格 |
...
> **说明**：示例使用北京地域镜像。其他地域请将镜像地址中的地域替换为云沙箱接入地域，并保持版本为 `v0.0.39`；`E2B_API_KEY`、`E2B_API_URL`、`E2B_DOMAIN` 也需通过环境变量（如 `.env` 或 `export`）配置为同一地域，SDK 会自动读取，无需在方法调用中显式传入。连接 CDP/VNC 端点时需在请求头携带 `X-Access-Token`：Python SDK 通过内部属性 `sbx._envd_access_token` 获取（后续版本可能重命名或移除），JS SDK 通过 `sbx.envdAccessToken` 获取（TypeScript 中为 `protected` 修饰，运行时可直接读取）。
...
FROM_IMAGE = "fc-e2b-registry.cn-beijing.cr.aliyuncs.com/runtime/browser:v0.0.39"
...
// SDK 自动读取 E2B_API_KEY / E2B_API_URL / E2B_DOMAIN 环境变量。
const FROM_IMAGE =
  'fc-e2b-registry.cn-beijing.cr.aliyuncs.com/runtime/browser:v0.0.39';
