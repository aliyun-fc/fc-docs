...
> **说明**：示例使用北京地域镜像。其他地域请将镜像地址中的地域替换为云沙箱接入地域，并保持版本为 `v0.0.52`；`E2B_API_KEY`、`E2B_API_URL`、`E2B_DOMAIN` 也需通过环境变量（如 `.env` 或 `export`）配置为同一地域，SDK 会自动读取，无需在方法调用中显式传入。示例通过 `network={"allow_public_traffic": True}` 开启公网访问，直接连接 CDP/VNC 端点。
...
browser 模板通过 3000 端口暴露以下端点，示例已开启公网访问，`<sandbox-host>` 通过 SDK 的 `sbx.get_host(3000)` 获取：
