...
> **说明**：示例使用北京地域镜像。其他地域请将镜像地址中的地域替换为云沙箱接入地域，并保持版本为 `v0.0.36`；`E2B_API_KEY`、`E2B_API_URL`、`E2B_DOMAIN` 也需通过环境变量（如 `.env` 或 `export`）配置为同一地域，SDK 会自动读取，无需在方法调用中显式传入。连接 CDP/VNC 端点时需在请求头携带 `X-Access-Token`：Python SDK 通过内部属性 `sbx._envd_access_token` 获取（后续版本可能重命名或移除），JS SDK 通过 `sbx.envdAccessToken` 获取（TypeScript 中为 `protected` 修饰，运行时可直接读取）。
