...
Claude Code 是 Anthropic 的终端 AI 编程 Agent，可在沙箱内读代码、改文件、执行命令。Claude Code 模板从官方 `claude-code` 镜像构建，预置 Claude Code CLI 与运行依赖，开箱可用。本页介绍能力、默认配置、构建与最小验证、使用限制；克隆仓库、连接 MCP、扩展 Skill、获取结构化输出等真实任务教程见[使用 Claude Code Sandbox](../../../05.最佳实践/05.使用%20Claude%20Code%20Sandbox.md)。
...
镜像不预装自定义 Skill。Claude Code 按以下文件系统约定发现 Skill：
...
可在提示词中写 `/skill-name` 显式调用，也可通过匹配 Skill 的 `description` 自动加载。完整机制见 [Claude Code Skills](https://code.claude.com/docs/zh-CN/skills)，真实任务接入示例见[使用 Claude Code Sandbox](../../../05.最佳实践/05.使用%20Claude%20Code%20Sandbox.md)。
...
- [使用 Claude Code Sandbox](../../../05.最佳实践/05.使用%20Claude%20Code%20Sandbox.md)（克隆仓库、MCP、Skill、结构化输出等真实任务完整教程）
- [内置模板](../02.内置模板.md)
