...
- 准备一个云沙箱可访问的 E2B 兼容镜像。首次体验可使用 [通过 OpenAPI 构建和管理模板](03.通过%20OpenAPI%20构建和管理模板.md) 中的官方镜像。
...
// SDK 自动读取 E2B_API_KEY / E2B_API_URL / E2B_DOMAIN 环境变量。
// 从镜像定义并构建模板。
const fromImage = process.env.FROM_IMAGE;
if (!fromImage) throw new Error("缺少环境变量: FROM_IMAGE");

const build = await Template.build(
  Template().fromImage(fromImage),
  `template-${Date.now()}`,
...
- [通过 OpenAPI 构建和管理模板](03.通过%20OpenAPI%20构建和管理模板.md)：了解自定义镜像、构建参数和模板构建。
