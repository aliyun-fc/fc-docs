...
镜像层数过多会影响构建和启动性能，建议合并 Dockerfile 中的构建步骤，压缩镜像层数。
...
脚本依赖 Docker 和 Python 3。运行前请确认源镜像已拉取到本地，并为目标镜像使用新的 tag。`docker export | docker import` 可能无法保留全部镜像元数据，使用压缩后的镜像前请先完成验证。
