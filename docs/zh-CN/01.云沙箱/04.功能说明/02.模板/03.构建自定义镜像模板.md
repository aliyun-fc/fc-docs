...
2. 为 ACR EE 实例添加专有网络，确保云沙箱可以通过 VPC 拉取镜像，并在该 VPC 下创建一个自建安全组（见下文「ACR EE 网络要求」）。
...
- VPC 下需至少配置一个自建的安全组，且该安全组的出方向规则需放行 443/TCP 端口，具体原因和详细要求见下方说明。
...
- **规则**：出方向规则需放行 443/TCP 端口，访问目标为 ACR EE 实例的 VPC 访问地址（形如 `xxx-registry-vpc.<地域>.cr.aliyuncs.com`）。在控制台新建安全组时默认出方向放行全部流量，无需改动即满足要求；安全组为有状态，无需额外配置入方向规则。
...
检查镜像地址、地域、命名空间、仓库权限、VPC 绑定和访问控制配置，并确认 VPC 下存在满足上述要求的自建安全组。使用私有 ACR EE 镜像时，网络配置错误比 SDK 参数错误更常见。
