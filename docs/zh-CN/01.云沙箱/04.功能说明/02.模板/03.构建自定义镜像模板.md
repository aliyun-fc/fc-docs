...
- VPC 下需至少配置一个自建的安全组，且该安全组规则至少需放开出方向 443/TCP，具体原因和详细要求见下方说明。
...
- **规则**：至少放开出方向 443/TCP，访问目标是 ACR EE 实例的 VPC 访问地址（形如 `xxx-registry-vpc.<地域>.cr.aliyuncs.com`）。在控制台新建安全组时默认出方向放行全部流量，无需改动即满足要求；安全组为有状态，无需额外配置入方向规则。
