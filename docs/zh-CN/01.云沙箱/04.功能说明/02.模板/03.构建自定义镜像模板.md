...
构建自定义镜像模板有两条路径，镜像要求和网络要求相同：

- **通过 E2B SDK 构建**：在本地运行 `Template.build`，适合首次验证和个人开发，是本文的主线。
- **通过 OpenAPI 构建**：调用 `CreateTemplate` 由服务端完成构建，适合自动化脚本、基础设施即代码（IaC）和平台化集成场景，参见[通过 OpenAPI 构建和管理模板](08.通过%20OpenAPI%20构建和管理模板.md)。

## 快速开始

### ACR EE 准备

使用云沙箱官方镜像时无需准备 ACR EE 实例，可参见下文[使用云沙箱官方镜像](#use-official-image)后直接构建。
...
默认情况下，无需额外配置即可完成构建。只有需要显式指定构建模式、目标镜像、私有仓库凭证或 ACR EE 网络时，才需要自定义构建行为，有两条等价途径：

- **OpenAPI（推荐）**：通过 `CreateTemplate` 的 `buildConfig`、`sandboxConfig.registryConfig`、`vpcConfig` 字段配置，字段说明参见[通过 OpenAPI 构建和管理模板](08.通过%20OpenAPI%20构建和管理模板.md)。
- **E2B SDK**：通过 `Template.build(..., headers={...})` 传入 `X-E2B-Template-*` 扩展 Header，见下文。

其中 `builder` 构建模式用于把普通业务镜像处理成可用于云沙箱模板的镜像。构建时，平台会临时启动一个函数计算函数，由它拉取源镜像、加入云沙箱运行所需的依赖，并推送为目标镜像；模板最终使用这个目标镜像创建沙箱运行环境。该临时函数创建在您自己的账号下，构建期间会产生少量函数计算费用。
...
| Header | 取值或格式 | 说明 |
| --- | --- | --- |
| `X-E2B-Template-Build-Mode` | `builder`、`direct` | 构建模式。`builder` 会使用临时函数计算函数生成适配云沙箱的目标镜像；`direct` 会直接使用源镜像。 |
...
- `builder`：使用临时函数计算函数处理源镜像，加入云沙箱运行依赖，并生成适配云沙箱的目标镜像，适合普通业务镜像。
