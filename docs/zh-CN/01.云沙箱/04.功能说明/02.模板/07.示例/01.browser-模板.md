...
> **说明**：示例中的 `E2B_API_KEY`、`E2B_API_URL`、`E2B_DOMAIN` 需在运行前通过环境变量（如 `.env` 或 `export`）配置为对应地域的接入地址，SDK 会自动读取，无需在方法调用中显式传入。连接 CDP/VNC 端点时需在请求头携带 `X-Access-Token`：Python SDK 通过内部属性 `sbx._envd_access_token` 获取（后续版本可能重命名或移除），JS SDK 使用公开字段 `sbx.envdAccessToken`。
