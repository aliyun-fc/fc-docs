...
- `runtime_config`：最终模板的运行规格。`cpu`（核数）、`memory_size`（MB）、`disk_size`（GB）按需填写，缺省时使用默认规格；`sandbox_config.image` 为必填的构建源镜像。
...
Python SDK 1.4.0 存在已知问题：`CreateTemplateSandboxConfig` 序列化时会无条件携带空 `steps` 数组，调用 CreateTemplate 将返回 `InvalidParameter`（`runtimeConfig.sandboxConfig.steps not supported yet`）。该问题只影响 Python SDK 的 CreateTemplate 调用，GetTemplate、ListTemplates、DeleteTemplate 不受影响；修复版本发布前，可暂时使用其他语言 SDK 创建模板。
...
=== "Java"
    ...
    CreateTemplateInput body = new CreateTemplateInput()
            ...
            .setRuntimeConfig(new CreateTemplateRuntimeConfig()
                    .setSandboxConfig(new CreateTemplateSandboxConfig()
                            .setImage("fc-e2b-registry.cn-beijing.cr.aliyuncs.com/runtime/code-interpreter-v1:v0.0.48")));
    String templateID = popClient
            .createTemplate(new CreateTemplateRequest().setBody(body))
            .getBody().getTemplateID();
    ```
...
=== "Java"
    ...
    ListTemplatesResponse response = popClient.listTemplates(
            new ListTemplatesRequest().setTeamID(teamID).setMaxResults(20));
