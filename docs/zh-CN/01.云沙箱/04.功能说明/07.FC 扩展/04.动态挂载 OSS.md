...
如需向运行中的 Sandbox 追加 OSS 挂载，请参见[追加挂载沙箱存储](./09.追加挂载沙箱存储.md)。
...
如果使用[第二代运行时（MicroVM）](../02.模板/03.通过%20OpenAPI%20构建和管理模板.md)的自定义模板，必须在构建模板前，在源镜像中预装 ossfs2 2.0.9 或以上版本。客户端安装、镜像构建和验证方法参见[挂载 OSS Volume](../12.卷/01.挂载%20OSS%20Volume.md)中的第二代运行时镜像要求。
