...
- 使用 `namedVolumeMounts` 时，引用的 Volume 已在同一个 Team 中创建并可用。当前可以使用 [OSS Volume](./07.创建%20OSS%20Volume.md)
  、[AgenticFS Volume](./08.创建%20AgenticFS%20Volume.md)或 [AgenticBucket Volume](./10.创建%20AgenticBucket%20Volume.md)。
- 创建 Sandbox 时，已通过 `createSandbox` 配置新增存储所需的执行 Role，以及 AgenticFS 所需的 VPC。该接口只追加挂载，不会补充或修改这些配置。

创建 Sandbox 的基本方法参见[创建沙箱](../01.沙箱/02.创建沙箱.md)。
...
| `agenticBucket` | `mountPoints` | 是 | 本次请求中普通 OSS 与 AgenticBucket 挂载点合计不能超过 5 个。 |
...
- 单次请求中的普通 OSS 与 AgenticBucket 挂载点合计不能超过 5 个；AgenticFS 挂载点最多 5 个。底层还会按 Sandbox 已有挂载与新增挂载的累计结果校验配额。
