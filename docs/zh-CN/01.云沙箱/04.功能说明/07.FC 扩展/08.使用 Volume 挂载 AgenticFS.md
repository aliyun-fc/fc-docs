...
- 已准备函数执行 RAM Role。该 Role 必须具备 AgenticFS 挂载和读写权限，且信任策略允许函数计算服务扮演。
...
函数执行 RAM Role 需要按实际读写需求授予 NAS Client 权限：
...
检查创建 Sandbox 时 `fc.sandbox.auth.role` 是否为空，以及该执行 Role 的信任策略是否允许函数计算服务扮演。
