...
AgenticFS Volume 用于将已有的 AgenticSpace 挂载到 Sandbox。挂载后，Sandbox 可以通过文件路径读写 AgenticFS，数据不随 Sandbox
释放而删除。适合保存工作区、模型产物和需要跨 Sandbox 复用的文件。
...
云沙箱 OpenAPI 当前支持 Java、Python、Go 和 TypeScript SDK。
支持范围、安装方式和版本以[阿里云 SDK & API 说明](https://help.aliyun.com/zh/functioncompute/alibaba-cloud-sdk-api-description)
为准。本文使用 Python 3.10 及以上版本演示。
...
请求参数、返回参数和错误码以 OpenAPI
门户的 [CreateVolume - 创建 Volume](https://api.aliyun.com/api/FCSandbox/2026-05-09/CreateVolume) 为准。
...
将挂载目录作为 `volume_mounts` 的 key，将 `CreateVolume` 返回的 `volume_name` 作为 value。以下代码接前一段继续执行，并在验证后清理资源：
...
`volume_mounts` 只引用 Volume 名称。删除 Volume 不会删除 AgenticFS 任何资源或其中的数据；如需继续使用该
Volume，请移除示例中的 `delete_volume`。
...
同一 Team 中已存在同名 Volume。更换 Volume 名称，或清理不再使用的同名 Volume。

### 如何挂载 Access Point 下的子目录

先将 Volume 根目录挂载到一个 Sandbox 中，通过 Sandbox 执行 `mkdir` 创建子目录，再将该子目录写入 `server_addr`：
