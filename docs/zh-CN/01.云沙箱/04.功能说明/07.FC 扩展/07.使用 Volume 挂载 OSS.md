...
- 调用 POP SDK 的阿里云身份具备 `fcsandbox:CreateVolume` 权限；查询 Volume 需要 `fcsandbox:ListVolumes` 和 `fcsandbox:GetVolume` 权限；运行本文的清理代码还需要 `fcsandbox:DeleteVolume` 权限。创建 Sandbox 时需要把执行 Role 传递给函数计算，因此该身份还需要对执行 Role 的 `ram:PassRole` 权限。
...
调用 Volume API 并创建 Sandbox 的 RAM 身份需要以下权限。`ram:PassRole` 用于在创建 Sandbox 时把执行 Role 传递给函数计算，请替换占位符：
...
函数执行 RAM Role 只需要目标 OSS 路径的访问权限，不需要 `ram:PassRole`：
...
生产环境应按实际资源范围收敛权限，请确保该 Role 的信任策略允许函数计算服务扮演。
