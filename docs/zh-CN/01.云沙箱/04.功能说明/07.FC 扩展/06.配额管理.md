...
云沙箱提供账号级和 Team 级两级配额。账号级配额限制阿里云账号在单个地域内可使用的函数计算资源总量；Team 级配额从账号额度中为指定 Team 预留 CPU 和内存。企业有多个团队、业务或环境共用云沙箱时，可以通过两级配额控制整体容量，并隔离不同 Team 的资源使用。

Team 配额通过阿里云 POP SDK 配置，使用阿里云 AccessKey 鉴权，E2B SDK 不支持该能力。创建 Team 和获取 Team ID，参见[创建 Team](../../01.开始使用/02.创建%20Team.md)。

## 配额模型

| 层级     | 生效范围           | 配额内容               | 管理方式                     |
|--------|----------------|--------------------|--------------------------|
| 账号级    | 阿里云账号的单个地域     | 函数计算账号级资源配额和使用限制   | 在配额中心查看，或按对应限制项的说明申请调整。   |
| Team 级 | 账号下的指定云沙箱 Team | 预留的 CPU 核数和内存 GB 数 | 通过云沙箱 POP SDK 配置。        |

账号级配额是地域内的整体资源边界，Team 级配额从账号额度中为指定 Team 预留 CPU 和内存。所有 Team 的 CPU 配额总和不能超过账号级 CPU 额度，内存配额总和不能超过账号级内存额度。实际创建 Sandbox 时，还会受到其他产品使用限制的约束。

```text
阿里云账号（账号级配额）
├── Team A（Team 级 CPU、内存预留配额）
│   └── Template、Sandbox、Volume 等 Team 资源
├── Team B（Team 级 CPU、内存预留配额）
└── Team C（Team 级 CPU、内存预留配额）
```

## 工作负载的资源隔离

同一账号内的强化学习、在线服务和 Agent 工具执行可能共享资源，但三类工作负载的资源使用方式不同。如果全部放在同一个 Team，批量任务或异常调用可能占满 Team 配额，影响其他业务。您可以按组织、环境或工作负载拆分 Team，并分别设置 CPU 和内存上限。Team 不需要与工作负载一一对应，应以实际隔离需求为准。

| 工作负载              | 资源使用特点                                   | 隔离建议                                              |
|-------------------|------------------------------------------|---------------------------------------------------|
| 强化学习（RL）          | 采样（Rollout）和评测通常批量运行，并发高且峰值明显。           | 对在线业务稳定性有要求时，可拆分为独立 Team，并根据并发 Sandbox 数量和规格设置配额。 |
| 在线服务（Serving）     | 模型或 Agent 的请求流量通常具有持续性，更关注可用性、延迟和资源余量。   | 可将生产、测试和批处理任务分开，避免非生产任务占用在线业务的 Team 配额。           |
| Agent 工具执行（Tools） | 代码、浏览器和 Shell 等工具的单次资源消耗差异较大，调用量也可能出现波动。 | 可按业务、租户或信任边界拆分 Team，通过独立配额控制异常调用的影响范围。            |

## 配额规划建议

规划 Team 配额时，先确认账号在目标地域的 CPU 和内存额度，再结合各工作负载的并发规模和单个 Sandbox 规格分配资源。Team 配额会占用账号级额度，应为业务增长和临时扩容保留未分配额度。

- 生产 Team、测试 Team 和批处理 Team 需要相互隔离时，分别设置配额。
- 避免让测试任务和生产任务共用同一个高配额 Team。
- 调低配额前，应先确认当前运行中的 Sandbox 和业务峰值。
- 删除配额前，应确认该 Team 不再需要独立配额。

## 配置账号级配额

函数计算的账号级资源边界包括已接入配额中心的配额，以及未接入配额中心的使用限制。前者可在配额中心查看和申请提升；后者需要根据对应限制项的说明，通过工单或产品支持申请调整。各项默认值和调整方式，请参见[配额与使用限制](https://help.aliyun.com/zh/functioncompute/limits-of-usage)。

## 配置 Team 级配额

Team 级配额以 Team ID 作为标识，通过 `TagValue` 传入。当前可以为 Team 预留 CPU 核数和内存 GB 数。
...
- 已开通函数计算和云沙箱。
- 已在阿里云控制台联系客服完成加白。
- 已按[创建 Team](../../01.开始使用/02.创建%20Team.md)在函数计算控制台创建 Team，并获取需要配置配额的 Team ID。
...
`UpdateQuota` 用于创建或覆盖指定 Team 的 CPU 和内存预留配额。配置时，所有 Team 的 CPU 配额总和不能超过账号级 CPU 额度，内存配额总和不能超过账号级内存额度。请求因超过配额被拒绝时，服务返回 429 / `LimitExceeded`。
...
在函数计算云沙箱控制台创建 Team 后，打开左侧 Team 选择器中的**管理 Team**，可在列表中查看并复制 Team ID。创建 Team 和获取 Team ID 的方法参见[创建 Team](../../01.开始使用/02.创建%20Team.md)。
...
- 配额用于限制资源使用上限，不能替代身份认证、RAM 授权和 API Key 管理。
