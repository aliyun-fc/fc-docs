...
Team 配额通过阿里云 POP SDK 管理，不通过 E2B SDK 的 Team API 管理。创建与访问 Sandbox 仍需云沙箱 API Key，在函数计算控制台完成；配额接口本身使用阿里云 AccessKey，不依赖 API Key。创建 Team、选择资源组与订阅计划的控制台操作，参见[Team 与订阅计划](../../01.开始使用/03.Team%20与订阅计划.md)。
...
- 已开通函数计算和云沙箱。
- 已在阿里云控制台联系客服完成加白。
- 已按[Team 与订阅计划](../../01.开始使用/03.Team%20与订阅计划.md)在函数计算控制台创建 Team，并获取需要配置配额的 Team ID。
...
在函数计算云沙箱控制台创建 Team 后，打开左侧 Team 选择器中的**管理 Team**，可在列表中查看并复制 Team ID。创建与管理步骤参见[Team 与订阅计划](../../01.开始使用/03.Team%20与订阅计划.md)。
