...
Team 配额通过阿里云 POP SDK 管理，不通过 E2B SDK 的 Team API 管理。创建与访问 Sandbox 仍需云沙箱 API Key，在函数计算控制台完成；配额接口本身使用阿里云 AccessKey，不依赖 API Key。创建 Team、选择资源组与订阅计划的控制台操作，参见[创建 Team](../../01.开始使用/02.创建%20Team.md)。
