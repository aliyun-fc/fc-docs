...
| `network.allowPublicTraffic` | 公网 URL 是否公开可达（`true` = 无需流量令牌），默认 `true` | 支持 | 否（仅创建时可设） |
...
> `egressProxy` **暂不支持**：请求体带该字段（非 `null`）时创建返回 `400` `network.egressProxy is not supported`，`updateNetwork` 同样返回 `400`。各客户端差异：Python SDK 2.31.0 无该参数，在 `network` 里写 `egress_proxy` 会被**静默丢弃**（不报错也不生效，沙箱按默认出网创建），切勿依赖；JS SDK 2.31.0 同样无该字段，但 2.41.0 起已提供并会上送（本页 `npm install e2b@^2.31.0` 会装到该版本），此时直接被服务端拒绝；直接调 REST 传入一律返回 `400`。`network.rules` 的详细用法见 [出网请求头转换](07.出网请求头转换.md)。

受限模式（`allowPublicTraffic: false`）下，公网入口（FC 数据面 ingress）的 403 门禁发生在 Host 改写之前：不带 `e2b-traffic-access-token` 的请求先被拒绝，需携带该令牌才能观察到 `maskRequestHost` 的改写效果。
...
> **注意**：设置 `allowPublicTraffic: false` 时必须同时设置 `secure: true`，否则创建请求会被直接拒绝并返回 `400`（`You cannot create a sandbox without public access unless you enable secure envd access via 'secure' flag.`）。`secure: true` 会启用控制面鉴权（`envdAccessToken`，请求头 `X-Access-Token`，由 SDK 自动携带），与公网入站的 `trafficAccessToken` 是两套独立令牌。
...
const sandbox = await Sandbox.create({
  ...
  secure: true, // allowPublicTraffic 为 false 时必须开启，否则创建请求直接返回 400
  network: {
...
sandbox = Sandbox.create(
    ...
    secure=True,  # allow_public_traffic 为 False 时必须开启，否则创建请求直接返回 400
...
- `egressProxy` **暂不支持**，REST 传入返回 `400`（Python SDK 2.31.0 无该参数）。
