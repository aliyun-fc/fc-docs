...
> `egressProxy` **暂不支持**，创建或更新时传入非 null 值返回 `400`。`network.rules` 的详细用法见 [出网请求头转换](07.出网请求头转换.md)。
...
> **注意**：显式 `secure: false` 与 `allowPublicTraffic: false` 组合在创建时返回 `400`。省略 `secure` 时默认开启保护，与显式 false 不同；示例中显式设置 true 便于理解。`secure: true` 会启用控制面鉴权（`envdAccessToken`，请求头 `X-Access-Token`，由 SDK 自动携带），与公网入站的 `trafficAccessToken` 是两套独立令牌。
...
const sandbox = await Sandbox.create({
  ...
  secure: true, // 与受限公网访问配合，显式保持 envd 保护开启
  network: {
...
try {
  ...
  if (!trafficAccessToken) throw new Error("服务未返回 trafficAccessToken");
...
- `true`（默认）：允许访问公网。
- `false`：等价于在 `denyOut` 中追加 `"0.0.0.0/0"`，建立「默认拒绝全部」的**基线**，而**不是不可覆盖的硬开关**。由于 `allowOut` 优先级高于 `denyOut`（见下文「优先级与匹配规则」），只有**单独使用（不配 `allowOut`）时才等于禁止一切出网**；一旦配合 `allowOut`，`allowOut` 命中的目标仍会放行，即可实现「默认拒绝 + 白名单」。
...
- `egressProxy` **暂不支持**，传入非 null 值返回 `400`。
...
- 运行中的沙箱支持替换出站规则与 `rules`。省略的可更新字段会被清空，不能按 PATCH 式合并理解。详细示例见[网络访问控制](05.%E7%BD%91%E7%BB%9C%E8%AE%BF%E9%97%AE%E6%8E%A7%E5%88%B6.md)。
- `egressProxy` 在 E2B 官方已定义；FC 当前不支持，创建或更新传入非 null 值返回 400，而不是静默忽略。
- 显式 `secure:false` 与 `allowPublicTraffic:false` 组合会在创建时返回 400。省略 `secure` 与显式 false 不同。
- FC HTTP `PUT /sandboxes/{id}/network` 的出网开关为 `allowInternetAccess`；当前 E2B 官方定义为 `allow_internet_access`。调用原始 HTTP 时按目标平台选择字段；SDK 映射需要与使用版本一起验证。
- `allowPublicTraffic` 和 `maskRequestHost` 属于 FC 创建时字段，不能在 PUT 网络更新时传入非 null 值。
...
[官方协议来源](https://docs.e2b.dev/api-reference/sandboxes/update-sandbox-network)
