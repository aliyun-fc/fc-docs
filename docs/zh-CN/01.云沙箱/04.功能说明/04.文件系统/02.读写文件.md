...
| Python 参数 | 默认值与返回行为 |
| --- | --- |
| `read(format="text")` | 返回 `str` |
| `read(format="bytes")` | 返回 `bytearray` |
| `read(format="stream")` | 返回 `FileStreamReader`，迭代产出 `bytes` |
| `user`, `request_timeout` | 操作用户、请求超时（秒） |
| `stream_idle_timeout` | 每个数据块之间的空闲超时；默认跟随请求超时，`0` 禁用 |
| `write(path, data)` | `data` 可为 `str`、`bytes`、文件对象；返回 `WriteInfo` |
| `write_files(files)` | 每项包含 `path`、`data`，返回 `List[WriteInfo]` |
| `gzip` | 默认 false；上传压缩需要 envd ≥ 0.5.7，不支持时回退 multipart |
| `use_octet_stream` | 默认 `None`，文件对象优先流式上传；需要 envd ≥ 0.5.7 |
| `metadata` | 文件扩展属性，需要 envd ≥ 0.6.2；不等同于沙箱 metadata |

流式读取的 `request_timeout` 只约束初始握手；读完会释放连接。提前停止时应使用上下文管理器或显式 `close()`。写入会覆盖已有文件并创建缺失的父目录。FC 是否可用仍需核对模板中的 envd 版本及文件访问限制。
