...
本文代码片段复用一个仍在运行的 `sandbox`。请先按[SDK 快速入门](../../01.开始使用/03.快速入门/01.通过%20SDK%20使用云沙箱.md)安装依赖、配置环境变量并创建沙箱，将片段放在该示例的 `try` 内执行，最后在 `finally` 中销毁沙箱。 Python 使用 `e2b_code_interpreter.Sandbox`，TypeScript 使用 `@e2b/code-interpreter`。
...
| `request_timeout` | 创建上下文或请求上下文服务的超时时间，Python 单位为秒；TypeScript 的 `requestTimeoutMs` 单位为毫秒。 |
