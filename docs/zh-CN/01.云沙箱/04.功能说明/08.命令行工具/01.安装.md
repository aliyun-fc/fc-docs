# 安装

E2B CLI 可用于在命令行中创建、查看、连接和终止云沙箱。云沙箱兼容 E2B CLI 的常用 Sandbox 命令，并扩展部分 Template 管理能力。

## 安装 CLI

macOS 可以使用 Homebrew 安装：

```bash
brew install e2b
```

也可以使用 npm 安装：

```bash
npm i -g @e2b/cli
```

安装后确认版本：

```bash
e2b --version
```

## 配置连接参数

E2B CLI 使用 `E2B_API_KEY` 完成认证。接入云沙箱时，还需要配置 API URL 和 Domain。API Key 的创建和管理方式参见[创建 API Key](../../01.开始使用/03.创建%20API%20Key.md)。

```bash
export E2B_API_KEY="<your-api-key>"
export E2B_API_URL="https://api.cn-beijing.e2b.fc.aliyuncs.com"
export E2B_DOMAIN="cn-beijing.e2b.fc.aliyuncs.com"
```

`E2B_ACCESS_TOKEN` 是 E2B 已废弃的旧认证变量。新版本 CLI 应使用 `E2B_API_KEY`，不要在新接入流程中依赖 `E2B_ACCESS_TOKEN`。

## 检查配置

配置完成后，可以列出 Sandbox 验证 CLI 是否能访问云沙箱：

```bash
e2b sandbox list
```

如果认证失败，请检查：

- API Key 是否按[创建 API Key](../../01.开始使用/03.创建%20API%20Key.md)创建。
- API Key 是否处于启用状态且未过期。
- `E2B_API_URL` 是否为 `https://api.cn-beijing.e2b.fc.aliyuncs.com`。
- `E2B_DOMAIN` 是否为 `cn-beijing.e2b.fc.aliyuncs.com`。
- E2B CLI 版本是否支持 `E2B_API_KEY` 认证。
- 当前账号是否具备函数计算相关操作权限。
