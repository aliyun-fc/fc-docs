...
`,
  ...
  // SDK 在 exitCode!==0 时会抛 CommandExitError，这里捕获并打印 stderr/stdout
  try {
    ...
    if (e?.name === "CommandExitError") {
      throw new Error(e.result.stderr || e.result.stdout);
    }
    throw e;
  }
} finally {
...
- 常用依赖如 `pandas`、`openpyxl`、绘图库和业务 SDK 建议放到模板。`code-interpreter-v1` 默认仅含 Python 与标准库，不含 pandas 等重型数据科学库，需要时请基于官方模板自定义。
- 对用户可见结果和排障日志分开处理，避免把完整堆栈或敏感数据直接返回。

## 可运行示例

可参考上方示例代码自行实现：在沙箱中写入 CSV 和分析脚本，并返回结构化业务摘要。
