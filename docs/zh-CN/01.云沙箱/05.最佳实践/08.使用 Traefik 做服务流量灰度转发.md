...
> 受限 API Key 在 Enforce 模式下需具有 `sandbox.network.ingress`（传 `network` 参数即会置位，本页示例显式设置以下入站参数）与 `sandbox.network.egress`（SDK 默认发送 `allow_internet_access`）权限。流量令牌应视为访问凭证，切勿写入日志或前端代码；需要限制访问时，参见[网络访问控制](../04.功能说明/06.网络/05.网络访问控制.md)与[责任共担](../06.安全合规/01.责任共担.md)。
...
- [网络访问控制](../04.功能说明/06.网络/05.网络访问控制.md)
