...
> 受限 API Key 在 Enforce 模式下需具有 `sandbox.network.ingress` 权限才能显式设置以下网络参数。
...
把上述步骤整合为一个可直接运行的脚本 `canary_demo.py`。相比操作步骤中的演示片段，该脚本做了工程化处理：以 `secure=True` 和 `allow_public_traffic=False` 创建沙箱，并自动附加 `e2b-traffic-access-token`、加入端口就绪轮询、对每一档权重做占比断言校验、并在 `finally` 中清理进程与沙箱。`E2B_API_KEY`、`E2B_DOMAIN`、`E2B_API_URL` 均从环境变量读取。
