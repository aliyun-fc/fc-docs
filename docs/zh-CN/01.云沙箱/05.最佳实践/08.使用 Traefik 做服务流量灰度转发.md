...
> 受限 API Key 在 Enforce 模式下需具有 `sandbox.network.ingress`（传 `network` 参数即会置位，本页示例显式设置以下入站参数）与 `sandbox.network.egress`（SDK 默认发送 `allow_internet_access`）权限。流量令牌应视为访问凭证，切勿写入日志或前端代码；需要限制访问时，参见[配置公网访问鉴权](../04.功能说明/06.网络/01.访问沙箱/01.访问沙箱%20HTTP%20端口.md#配置公网访问鉴权)与[责任共担](../06.安全合规/01.责任共担.md)。
...
- 沙箱需要能访问公网以下载 Traefik 二进制（默认使用 Serverless OSS 全球下载域名，见「常见问题」）。
...
# 公网入口启用流量令牌鉴权，SDK 通信保持 secure=True。
...
> **说明**：示例设置 `secure=True` 保护 SDK 通信，并设置 `network={"allow_public_traffic": False}` 对公网入口启用流量令牌鉴权。外部请求使用 `sbx.traffic_access_token`，通过 `e2b-traffic-access-token` 请求头传入。
...
# 解压前强制校验 SHA-256，拒绝被篡改或不完整的下载内容
if not SHA256:
    raise SystemExit("必须设置 TRAEFIK_SHA256")
...
# Traefik 二进制来自 Serverless OSS 全球下载域名。可用 TRAEFIK_TARBALL 环境变量覆盖为内网镜像；
# 覆盖时必须通过 TRAEFIK_SHA256 提供匹配的 SHA-256，否则完整性校验不通过。
...
**1. Traefik 下载缓慢或失败？**
默认下载地址指向 Serverless OSS 全球下载域名，全球可达，不依赖 GitHub。建议：

- 下载完成后会按内置 SHA-256 强制校验完整性。若通过 `TRAEFIK_TARBALL` 环境变量覆盖了下载地址（例如指向自建对象存储 / 内网镜像），则**必须**同时通过 `TRAEFIK_SHA256` 提供匹配的 SHA-256，以保证完整性校验仍能通过。
- **生产推荐做法**：把 Traefik 二进制**预置进自定义沙箱模板镜像**，运行时无需再下载即可直接使用（构建方式参见 [使用 GHCR 自定义镜像构建 Sandbox 模板](./07.使用%20GHCR%20自定义镜像构建%20Sandbox%20模板.md)）。

**2. 端口访问返回 5xx？**
确认对外暴露的端口在 `[3000, 65535]` 范围内，且沙箱内对应服务已经正常监听、就绪。

**3. 生产环境如何保证访问安全？**
设置 `secure=True` 和 `network={"allow_public_traffic": False}`。访问动态端口时，通过 `e2b-traffic-access-token` 请求头传入 `sbx.traffic_access_token`。切勿把访问令牌写入日志或前端代码。
...
把上述步骤整合为一个可直接运行的脚本 `canary_demo.py`。相比操作步骤中的演示片段，该脚本做了工程化处理：以 `secure=True` 和 `network={"allow_public_traffic": False}` 创建沙箱，并自动附加 `e2b-traffic-access-token`、加入端口就绪轮询、对每一档权重做占比断言校验、并在 `finally` 中清理进程与沙箱。`E2B_API_KEY`、`E2B_DOMAIN`、`E2B_API_URL` 均从环境变量读取。
...
def auth(sbx: Sandbox) -> dict:
    """启用流量令牌的沙箱访问动态端口需带 e2b-traffic-access-token；未启用时 traffic_access_token 为 None。"""
    token = sbx.traffic_access_token
...
def check_ratio(sbx: Sandbox, w1: int, w2: int) -> None:
    ...
    # 先断言所有请求都被归类为 v1/v2（无异常/other），再做占比校验
    assert hits["v1"] + hits["v2"] == N_REQUESTS, f"存在异常命中: {dict(hits)}"
    exp_v1 = w1 / (w1 + w2) * 100
...
- [访问沙箱 HTTP 端口](../04.功能说明/06.网络/01.访问沙箱/01.访问沙箱%20HTTP%20端口.md)
- [配置公网访问鉴权](../04.功能说明/06.网络/01.访问沙箱/01.访问沙箱%20HTTP%20端口.md#配置公网访问鉴权)
- [出网流量控制](../04.功能说明/06.网络/02.出网流量控制.md)
- [责任共担](../06.安全合规/01.责任共担.md)
