...
除了这三个 envd 相关条件外，镜像还必须满足平台的硬性要求，详见[通过 OpenAPI 构建和管理模板](../04.功能说明/02.模板/08.通过%20OpenAPI%20构建和管理模板.md)：架构为 `linux/amd64`、ACR EE 未开启镜像加速、`/etc/passwd` 和 `/etc/group` 可写，以及 `commands.run` 和 PTY 需要固定路径的 `/bin/bash`。
