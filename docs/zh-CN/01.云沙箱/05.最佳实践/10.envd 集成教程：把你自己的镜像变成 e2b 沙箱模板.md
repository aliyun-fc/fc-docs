...
除了这三个 envd 相关条件外，镜像还必须满足平台的硬性要求，详见[通过 OpenAPI 构建和管理模板](../04.功能说明/02.模板/03.通过%20OpenAPI%20构建和管理模板.md)：架构为 `linux/amd64`、ACR EE 未开启镜像加速、`/etc/passwd` 和 `/etc/group` 可写，以及 `commands.run` 和 PTY 需要固定路径的 `/bin/bash`。
...
sbx = Sandbox.create(
    template="envd-all-in-one-4c8g-v2",
    timeout=900,
    api_key=os.environ["E2B_API_KEY"],
    api_url=os.environ["E2B_API_URL"],
    domain=os.environ["E2B_DOMAIN"],
...
try:
    ...
    for i in range(12):
        ...
        raise TimeoutError("envd 在重试窗口内未就绪")
...
|---|---|---|
| envd 二进制 | `COPY --from=base:v0.0.52 /.fce2b/envd` | 同左（静态链接，跨发行版可用） |
