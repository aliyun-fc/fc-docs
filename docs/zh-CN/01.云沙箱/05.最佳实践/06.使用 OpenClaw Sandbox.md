...
> **访问说明**：`allow_public_traffic` 默认为 `True`，`secure` 默认为 `True`，分别控制公网流量鉴权和 SDK/envd 鉴权；示例省略默认网络参数。公网 URL 应视为访问凭证：Gateway URL 含应用层 Token，持有完整 URL 的人可访问 Control UI，勿公开分享。需要限制访问时，参见[网络访问控制](../04.功能说明/06.网络/05.网络访问控制.md)；显式设置网络入站参数的受限 API Key 需具有 `sandbox.network.ingress` 权限。浏览器 WebSocket 不能自定义流量令牌请求头，受限模式应通过受控后端代理注入 `e2b-traffic-access-token`，不能改用 query 参数。
...
若已按上文 [部署 Gateway](#gateway) 创建了 Sandbox，直接在同一个 `sandbox` 上执行下方命令即可，**无需再次创建**。
...
**第二步：SDK 改 `OPTS`。** 将 [OpenClaw 模板](../04.功能说明/02.模板/07.其他模板/05.OpenClaw%20模板.md#openclaw) 中的默认 `OPTS`：
...
完成自定义域名配置并 [部署 Gateway](#gateway) 后，示例沿用默认的公网免令牌访问，Control UI 使用 OpenClaw 自身的 Gateway Token 鉴权：
...
若出现「浏览器来源不被允许」，确认 `allowedOrigins` 与地址栏来源完全一致（`https://{sandbox.get_host(PORT)}`，无尾部 `/`），修改后 [重启 Gateway](#gateway_1)。
...
以下示例假设已按上文创建 `sandbox`（Gateway 或 Agent CLI 均可）。镜像不预装自定义 Skill；可用 `openclaw skills list` 查看已加载 Skill（含 bundled）。路径约定见 [OpenClaw 模板](../04.功能说明/02.模板/07.其他模板/05.OpenClaw%20模板.md#openclaw)。
