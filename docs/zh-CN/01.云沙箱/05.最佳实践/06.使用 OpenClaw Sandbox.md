...
# 官方文档亦支持 OPENCLAW_GATEWAY_TOKEN 环境变量，本质相同。
...
要在浏览器中打开云沙箱的 Gateway Control UI，**须先绑定自定义域名**。默认平台域名（`*.e2b.fc.aliyuncs.com`）会**触发下载而非打开页面**；绑定自定义域名后可正常访问。控制台添加域名、DNS 解析、HTTPS 证书及 SDK 配置的完整步骤，见 [云沙箱自定义域名](../04.功能说明/07.FC%20扩展/03.自定义域名.md)。

### 配置自定义域名

**第一步：控制台配置。** 在函数计算控制台为云沙箱绑定自定义域名，配置证书与 DNS（`api.<你的自定义域名>` / `*.<你的自定义域名>`）。

**第二步：SDK 改 `OPTS`。** 将 [OpenClaw 模板](../04.功能说明/02.模板/07.其他模板/05.OpenClaw%20模板.md#build-template) 中的默认 `OPTS`：
...
完成自定义域名配置并 [部署 Gateway](#deploy-gateway) 后，示例沿用默认的公网免令牌访问，Control UI 使用 OpenClaw 自身的 Gateway Token 鉴权：

| 鉴权 | 来源 | 作用 | 传递方式 |
|------|------|------|----------|
| Gateway Token | 代码中自行设定的 `TOKEN` | OpenClaw Control UI | URL 参数 `?token=` |

> **重要**：`?token=` 中的 Gateway Token 是应用层凭证，会进入网关访问日志、浏览器历史与 `Referer`。切勿把含 Token 的 Gateway URL 写入日志或前端代码，也不要公开分享；应最小化使用范围并及时轮转 Token。

1. 运行上文脚本，记录输出的 `Gateway` URL。
2. 打开 Gateway URL（URL 中已含 `?token=`）。
...
以下示例假设已按上文创建 `sandbox`（Gateway 或 Agent CLI 均可）。镜像不预装自定义 Skill；可用 `openclaw skills list` 查看已加载 Skill（含 bundled）。路径约定见 [OpenClaw 模板](../04.功能说明/02.模板/07.其他模板/05.OpenClaw%20模板.md#skill-capability-notes)。
...
| Gateway 浏览器打不开 | 先绑定自定义域名并使用该域名访问；确认 URL 含 `?token=` |
| 浏览器触发下载 | 须绑定自定义域名，见 [云沙箱自定义域名](../04.功能说明/07.FC%20扩展/03.自定义域名.md) 与 [浏览器访问（云沙箱）](#browser-access) |
