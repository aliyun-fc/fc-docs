...
print(f"Gateway: {url}")
# X-Access-Token 在 Sandbox.create 成功后由 SDK 返回，用于云沙箱公网端口代理鉴权
...
完成自定义域名配置并 [部署 Gateway](#gateway) 后，公网访问还需两层鉴权：

| 鉴权 | 来源 | 作用 | 传递方式 |
|------|------|------|----------|
| Sandbox Access Token | `Sandbox.create` 返回的 `sandbox._envd_access_token` | 云沙箱端口代理 | 请求头 `X-Access-Token` |
...
可用 `curl` 先验证（须带 `X-Access-Token`，否则云沙箱返回 403）：
