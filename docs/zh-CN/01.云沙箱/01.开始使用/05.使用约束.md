...
本文档示例中的安装命令代表对应示例的验证版本。不同示例可能来自不同验证批次，接入时需要同时关注安装命令和[E2B 兼容说明](06.E2B%20兼容说明.md)中的能力边界。
...
Sandbox、Commands、Filesystem、Code Interpreter、Template 和 CLI 的接口边界集中在[E2B 兼容说明](06.E2B%20兼容说明.md)中维护。接入前应确认业务依赖的接口已在支持范围内；未兼容能力不应作为云沙箱接入主路径。
