...
本文档示例中的安装命令代表对应示例的验证版本。不同示例可能来自不同验证批次，接入时需要同时关注安装命令和[E2B 兼容说明](07.E2B%20兼容说明.md)中的能力边界。
...
本文档中使用顶层 `await` 的 TypeScript 示例需要 ES Module 项目：在项目目录运行 `npm pkg set type=module`，或将示例保存为 `.mts` 文件后使用 `npx tsx example.mts` 执行。独立代码片段应放入相应完整示例的运行上下文中。

使用 Snapshot（含按 `name` 过滤列表）时，需要更高 SDK 版本：Python `e2b>=2.33.0`，TypeScript `e2b>=2.34.0`。详见[快照](../04.功能说明/01.沙箱/12.快照.md)。
...
API Key 需要先在函数计算控制台创建。前提条件、创建步骤和管理操作参见[创建 API Key](03.创建%20API%20Key.md)。
...
API Key 应在函数计算控制台创建、查看、编辑、重置、禁用和删除，不通过 E2B SDK 管理。已禁用或已过期的 API Key 不能通过数据面认证。不要在日志、截图、工单或前端页面中暴露完整 API Key。管理建议参见[创建 API Key](03.创建%20API%20Key.md)。
...
更多模板构建要求参见[通过 OpenAPI 构建和管理模板](../04.功能说明/02.模板/03.通过%20OpenAPI%20构建和管理模板.md)。
...
Sandbox、Commands、Filesystem、Code Interpreter、Template 和 CLI 的接口边界集中在[E2B 兼容说明](07.E2B%20兼容说明.md)中维护。接入前应确认业务依赖的接口已在支持范围内；未兼容能力不应作为云沙箱接入主路径。
