...
| **方式** | **适用场景** | **前置条件** |
| --- | --- | --- |
| [方法一：登录实例，通过 git clone 安装（推荐）](#git-clone) | 准确地逐个安装开源插件。安装速度快、成功率高且流程可控，但操作较繁琐 | 启动工作站，且已获取缺失插件的GitHub地址（可通过ComfyUI Manager获取） |
| [方法二：通过 ComfyUI Manager 安装](#comfyui-manager) | 通过图形界面一键安装、管理开源插件。操作简单，但安装速度较慢、成功率较低 | 启动工作站 |
| [方法三：通过文件管理安装](#方法三通过文件管理安装) | 插件批量迁移、闭源插件导入、网络受限下开源插件安装 | 其他ComfyUI环境中已有一批插件，或已有闭源插件源码包 |
...
4. **安装插件依赖**
  ...
  - 手动安装：参考[方法一：登录实例，通过 git clone 安装（推荐）](#git-clone)
...
5. **重启验证与排错**
  ...
  - 处理冲突（若有）：开源插件间可能存在 Python 库的依赖冲突。虽然**一键依赖安装**会尽量缓解此类问题，但无法保证 100% 成功。
    ...
    - 若日志显示某插件导入失败（`Failed to import`），请根据[方法一](#git-clone)和报错信息，结合该插件的官方文档手动修复依赖。
