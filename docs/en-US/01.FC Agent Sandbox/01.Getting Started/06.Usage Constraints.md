...
Snapshot APIs (including list filtering by `name`) require newer SDK versions: Python `e2b>=2.33.0` and TypeScript `e2b>=2.34.0`. For details, see [Snapshots](../04.Features/01.Sandbox/12.Snapshots.md).
...
Create API keys in the Function Compute console first. For prerequisites, creation steps, and management operations, see [Create an API Key](03.Create%20an%20API%20Key.md).
...
Create, view, edit, reset, disable, and delete API keys in the Function Compute console, not through the E2B SDK. Disabled or expired API keys cannot authenticate to the data plane. Do not expose full API key values in logs, screenshots, support tickets, or frontend pages. For management recommendations, see [Create an API Key](03.Create%20an%20API%20Key.md).
...
Pause and resume is currently available only to allowlisted accounts. Without allowlist access, call `sandbox.kill()` to release resources.
