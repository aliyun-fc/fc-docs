...
The FC Agent Sandbox data plane is compatible with the E2B SDK and E2B CLI and does not provide a standalone data-plane SDK. For data-plane integration, continue to use E2B SDK and CLI objects, methods, and parameter names, and switch authentication and service endpoints to FC Agent Sandbox. Resource management capabilities such as Team, API Key, and Quota belong to the control plane and are available through native OpenAPI, Alibaba Cloud SDK, or Alibaba Cloud CLI. For details, see [Alibaba Cloud SDK and API Guide](04.Alibaba%20Cloud%20SDK%20and%20API%20Guide.md).
...
API keys must be created and managed in the Function Compute console. For instructions, see [Create an API Key](../01.Getting%20Started/03.Create%20an%20API%20Key.md). Do not manage FC Agent Sandbox API keys through the E2B SDK, and do not write API keys into source repositories, logs, screenshots, or frontend pages.
...
- `timeout`: Sandbox lifetime in seconds. The current Python SDK create default is 300 seconds. See the parameter crosswalk for FC limits and zero-value behavior; the raw upstream create API default is 15 seconds.
...
- `timeoutMs`: Sandbox lifetime in milliseconds, distinct from command execution and client request timeouts.
- `envs`: environment variables injected when creating the sandbox.
...
- FC Agent Sandbox API key and access token management: handled through the Function Compute console, not through the E2B SDK. For instructions, see [Create an API Key](../01.Getting%20Started/03.Create%20an%20API%20Key.md).
- Volume and Team management: Volume is not currently part of the FC Agent Sandbox E2B compatibility path; manage Teams in the Function Compute console.
- Snapshot: supported. For details, see [Snapshots](../04.Features/01.Sandbox/12.Snapshots.md).
- Metrics and Logs have field/result limitations. Network Config Update supports a defined subset of outbound settings; see Network Access Control.

For the full capability boundary, see [E2B SDK-compatible API List](01.E2B%20SDK-compatible%20API%20List.md) and [Getting Started / E2B Compatibility](../01.Getting%20Started/07.E2B%20Compatibility.md).

## Connection and naming

Use the FC API Key, regional API URL and sandbox domain described in [SDK connection parameters](02.E2B%20SDK%20Connection%20Parameters.md). Keep SDK option names separate from HTTP fields.

| Meaning | Python | TypeScript | HTTP |
| --- | --- | --- | --- |
| Template | `template` | first argument to `Sandbox.create()` | `templateID` |
| Sandbox lifetime | `timeout`, seconds | `timeoutMs`, milliseconds | `timeout`, seconds |
| Client request timeout | `request_timeout` | `requestTimeoutMs` | Client setting, not the sandbox lifetime |
| Environment variables | `envs` | `envs` | `envVars` |
| Sandbox metadata | `metadata` | `metadata` | `metadata`, string map |
| Sandbox identity | `sandbox_id` | `sandboxId` | `sandboxID` |
| Outbound access | `allow_internet_access` | `allowInternetAccess` | Create: `allow_internet_access`; FC also accepts camelCase |

SDK defaults and optional parameters are not the same as HTTP schema defaults and required fields. Names were checked against Python SDK 2.37.1 and TypeScript SDK 2.38.2; FC examples may target an older tested version.
