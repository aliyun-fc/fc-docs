...
This topic covers common error categories for the E2B-compatible data plane. When troubleshooting, combine E2B SDK exceptions, HTTP status codes, CLI output, and the FC Agent Sandbox compatibility boundary to identify the issue type. For native control-plane OpenAPI errors and entry points, see [Alibaba Cloud SDK and API Guide](04.Alibaba%20Cloud%20SDK%20and%20API%20Guide.md).
...
- Whether `E2B_API_KEY` is an FC Agent Sandbox API key created as described in [Create an API Key](../01.Getting%20Started/03.Create%20an%20API%20Key.md).
...
- Logs: limited capability; do not rely on it for production log collection.
- Network Config Update: supports a subset of outbound rules and transforms; unsupported non-null fields return 400. See [Network Access Control](../04.Features/06.Network/05.Network%20Access%20Control.md).
...
- Snapshots: supported. For details, see [Snapshots](../04.Features/01.Sandbox/12.Snapshots.md). Name conflicts, deleting a Snapshot that is still in use, and overriding disallowed environment variables or mount settings when creating a Sandbox from a Snapshot fail; troubleshoot against the Snapshot documentation.
- Volume, Team management, and API key / access token management: not currently part of the FC Agent Sandbox E2B compatibility path.
