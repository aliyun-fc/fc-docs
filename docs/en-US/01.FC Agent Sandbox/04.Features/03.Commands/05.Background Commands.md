...
Python `background=True` returns `CommandHandle`. `wait()` waits for completion; `disconnect()` stops receiving events without killing the process. Reconnect with `sandbox.commands.connect(pid)`. `kill()` uses SIGKILL and returns whether the process was killed. The background overload takes no stdout/stderr callbacks; pass them to `wait()`.
