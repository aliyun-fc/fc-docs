...
- The VPC must contain at least one security group you created, and its outbound rules must allow TCP port 443. See the details below.
...
- **Rules**: outbound rules must allow TCP port 443 to the VPC endpoint of the ACR EE instance (for example, `xxx-registry-vpc.<region>.cr.aliyuncs.com`). A security group created in the console allows all outbound traffic by default, which meets the requirement without changes. Security groups are stateful, so no inbound rules are needed.
