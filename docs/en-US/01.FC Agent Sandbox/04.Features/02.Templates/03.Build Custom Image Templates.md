...
2. Attach a VPC to the ACR EE instance so FC Agent Sandbox can pull images over VPC, and create your own security group in that VPC (see "ACR EE network requirements" below).
...
- The VPC has at least one security group you created yourself, and its outbound rules must at least allow TCP port 443. See the details below.

Details about the user-created security group:

- **Why a user-created security group is required**: the security group automatically created when the ACR EE instance is attached to a VPC is managed by the cloud service and cannot be referenced by Function Compute. Template builds create a temporary builder function under your account that pulls and pushes images through the VPC, and the security group attached to that function must be a user-created (not service-managed) one.
...
- **Rules**: at a minimum, allow outbound TCP port 443 to the VPC endpoint of the ACR EE instance (for example, `xxx-registry-vpc.<region>.cr.aliyuncs.com`). A security group created in the console allows all outbound traffic by default, which meets the requirement without changes. Security groups are stateful, so no inbound rules are needed.
...
Check the image address, region, namespace, repository permissions, VPC binding, and access control configuration, and confirm that a qualifying user-created security group exists in the VPC. When using a private ACR EE image, network configuration errors are more common than SDK parameter errors.
