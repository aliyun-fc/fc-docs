...
2. Attach a VPC to the ACR EE instance so FC Agent Sandbox can pull images over VPC, and create a security group that is not managed by a cloud service in that VPC (see "ACR EE network requirements" below).
...
- The VPC has at least one security group that is not managed by a cloud service, and its rules allow access to the corresponding ACR EE instance. See the details below.

Details about the security group that is not managed by a cloud service:

- **Why a self-managed security group is required**: the security group automatically created when the ACR EE instance is attached to a VPC is managed by the cloud service and cannot be referenced by Function Compute. Template builds create a temporary builder function under your account that pulls and pushes images through the VPC, and the security group attached to that function must be a user-created (not service-managed) one.
- **Location**: create it in the access VPC attached to the ACR EE instance (the VPC configured under Access Control in the ACR EE console). It does not need to be bound to any ECS instance.
- **Rules**: outbound rules must allow access to the VPC endpoint of the ACR EE instance (for example, `xxx-registry-vpc.<region>.cr.aliyuncs.com`) on TCP port 443. A security group created in the console allows all outbound traffic by default, which meets the requirement without changes. Security groups are stateful, so no inbound rules are needed.

If no qualifying security group exists in the VPC, the template build fails (for example, with `acree need to have at least one vpc which is running and has security group that is not service managed`) or falls back to accessing ACR EE over the public network, which usually shows up as a pull timeout. You can also explicitly specify the VPC, vSwitch, and security group through the `X-E2B-Template-Source-VPC-ID` headers (see "Advanced configuration" below), but an explicitly specified security group must meet the same requirements.
...
Check the image address, region, namespace, repository permissions, VPC binding, and access control configuration, and confirm that a qualifying security group that is not managed by a cloud service exists in the VPC. When using a private ACR EE image, network configuration errors are more common than SDK parameter errors.
