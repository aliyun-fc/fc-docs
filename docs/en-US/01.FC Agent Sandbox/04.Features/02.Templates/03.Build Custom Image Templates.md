...
`sandbox-gateway` supports passing `X-E2B-Template-*` extension headers through `Template.build(..., api_headers={...})`. You only need to configure these headers when you need to explicitly specify the build mode, target image, private registry credentials, or ACR EE network settings.
