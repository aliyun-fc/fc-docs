...
- Keep the VPC binding of the ACR EE instance stable during template builds. After a VPC is unbound, the entry ENI is deleted, but private DNS resolution inside the VPC may still point to the old ENI IP, causing the build to time out as a network black hole.
...
### 3. Build fails with `template build failed: envd inject failed`

This message is the stable, desensitized status shown for the template; it does not mean envd injection itself failed. The underlying cause is usually in image pulling or the network path, for example missing private registry credentials or a stale ACR EE VPC entry ENI. Check the build log for details.

If the build log contains `dial tcp <IP>:443: i/o timeout`, the VPC entry ENI of the ACR EE instance is likely gone. A security group block shows up as `i/o timeout`, and a missing entry ENI — where DNS still returns the old IP — shows the same `i/o timeout` (black hole), so the two cannot be distinguished from the error alone.

To diagnose: resolve the `-vpc` domain of the ACR EE instance inside the VPC to get the entry IP, then check in the ECS ENI console whether an ENI with that private IP still exists. As a control, dial TCP 443 to a known-alive private IP in the same VPC; a `connection refused` response proves the VPC networking itself is fine and the problem is the missing entry ENI.

To fix: re-bind the VPC and vSwitch in the ACR EE console to recreate the entry ENI, then rebuild the template. See "ACR EE network requirements" above.

### 4. vSwitch in unsupported zone
...
### 5. Sandbox created successfully but `run_code` fails
