...
> **Note**: The example image is hosted in the Singapore region (`ap-southeast-1`); when building and running the template, set the `E2B_API_URL` and `E2B_DOMAIN` environment variables to the same region's endpoints (the SDK reads them automatically, no need to pass them explicitly in method calls). Local environment setup and how to obtain `X-Access-Token` are the same as in the [Browser Template Example](01.Browser%20Template.md).
...
The browser part (create sandbox, wait for `/health`, connect over CDP, take a screenshot) is identical to the [Browser Template Example](01.Browser%20Template.md). The additional capability of the All-In-One template is executing code through Code Interpreter within the same sandbox: after the browser phase, use `run_code` / `runCode` to process the browser artifacts.

**Python**:

```python
# Continuing with sbx from the browser template example (same sandbox), run a Python snippet to verify code execution.
...
- [Browser Template Example](01.Browser%20Template.md) (choose when you only need browser automation capabilities)
