...
The image does not preload custom Skills. Claude Code discovers Skills through these filesystem conventions:
...
Call `/skill-name` explicitly in the prompt, or let Claude Code load a Skill automatically when its `description` matches the task. For the complete mechanism, see [Claude Code Skills](https://code.claude.com/docs/en/skills). For real-task integration examples, see [Use Claude Code Sandbox](../../../05.Best%20Practices/05.Use%20Claude%20Code%20Sandbox.md).
