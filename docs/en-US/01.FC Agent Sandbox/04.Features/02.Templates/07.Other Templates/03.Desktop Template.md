...
This page uses the Beijing-region image at version `v0.0.52`:

| **Configuration** | **Default** | **Description** |
| --- | --- | --- |
| Container image | `fc-e2b-registry.cn-beijing.cr.aliyuncs.com/runtime/desktop:v0.0.52` | Official Desktop image, Beijing region example |
| CPU | 4 vCPU | Recommended starting spec |
| Memory | 8192 MB | Recommended starting spec |

> **Note**: When building a template, the image registry region must match the FC Agent Sandbox access region. For other regions, replace the region segment in the image URL accordingly and keep the version tag `v0.0.52`. Also configure `E2B_API_URL` and `E2B_DOMAIN` for the same region.
...
FROM_IMAGE = "fc-e2b-registry.cn-beijing.cr.aliyuncs.com/runtime/desktop:v0.0.52"
...
const FROM_IMAGE =
  "fc-e2b-registry.cn-beijing.cr.aliyuncs.com/runtime/desktop:v0.0.52";
...
`stream.get_url()` returns the full URL of the built-in noVNC page (`/vnc.html`) served by the image on port 6080; the page connects to `/websockify` on the same origin automatically, so no extra `path` parameter is needed. `/static/vnc.html` returns 404 in the Desktop image. The default platform domain (`*.e2b.fc.aliyuncs.com`) **triggers a download instead of opening the page**, so [bind a custom domain](../../07.FC%20Extensions/03.Custom%20Domains.md) first. `get_url(auth_key=...)` requires `stream.start(require_auth=True)` first to generate the stream access key; otherwise `get_auth_key()` raises a `RuntimeError`.
...
| Live stream | Only one full-desktop stream can be active per sandbox at a time; specifying a single window is not supported in version `v0.0.52` |
...
- [Browser Template](01.Browser%20Template.md)
- [All-In-One Template](02.All-in-One%20Template.md)
