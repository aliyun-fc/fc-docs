...
> **Note**: The example uses the Beijing image. For another region, replace the region segment in the image address with the FC Agent Sandbox endpoint region and keep the `v0.0.52` tag. Set `E2B_API_KEY`, `E2B_API_URL`, and `E2B_DOMAIN` (through `.env` or `export`) to that same region. The SDK reads them automatically, so you do not need to pass them into method calls. The examples enable public access with `network={"allow_public_traffic": True}` and connect directly to the CDP/VNC endpoints.
...
The browser template exposes the following endpoints through port 3000. The examples enable public access, and `<sandbox-host>` is obtained through the SDK's `sbx.get_host(3000)`:
