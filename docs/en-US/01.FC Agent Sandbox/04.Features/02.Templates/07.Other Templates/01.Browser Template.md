...
> **Access**: `allow_public_traffic` and `secure` both default to `True`; they control public traffic authentication and SDK/envd authentication respectively. The examples omit the default network setting. Treat public URLs as access credentials: without application authentication, anyone with the URL can control the browser or desktop. Do not share them publicly. See [Network Access Control](../../06.Network/05.Network%20Access%20Control.md) for restricted access. Restricted API keys need `sandbox.network.ingress` permission to set ingress options explicitly. Browser WebSocket clients cannot set traffic-token headers; use a controlled backend proxy to inject `e2b-traffic-access-token` in restricted mode. Query parameters are not supported.
