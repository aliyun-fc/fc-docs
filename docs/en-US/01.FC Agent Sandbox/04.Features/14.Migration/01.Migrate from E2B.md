...
If your application depends on E2B Volume, MCP gateway, BYOC, advanced network configuration, or high-frequency metrics, redesign those parts before migration instead of moving them over directly. Snapshots are supported, but naming, deletion paths, and restore constraints are not identical to official E2B; see [Snapshots](../01.Sandbox/12.Snapshots.md). Do not carry over official E2B Snapshot deletion calls unchanged. FC Agent Sandbox supports sandbox metrics and currently returns them at 1-minute granularity.
