...
- It must provide a `linux/amd64` variant, `/bin/bash` at that exact path, and writable standard `/etc/passwd` and `/etc/group` files.
...
- Do not set an FC Agent Sandbox runtime entry point in the source image. For a regular application image, the `Template.build()` builder mode injects the required runtime dependencies. See [Build Custom Image Templates](../02.Templates/03.Build%20Custom%20Image%20Templates.md) for the complete image, ACR EE network, and builder requirements.
