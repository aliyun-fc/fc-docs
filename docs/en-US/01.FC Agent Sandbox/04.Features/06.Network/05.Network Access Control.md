...
| `network.allowPublicTraffic` | Whether the public URL is publicly reachable (`true` = no traffic token required); defaults to `true` | Yes | No (can only be set at creation) |
...
> `egressProxy` is **not yet supported**: a non-`null` value in the request body makes create return `400` `network.egressProxy is not supported`, and `updateNetwork` returns `400` as well. Client differences: Python SDK 2.31.0 has no such field and **silently drops** an `egress_proxy` key in `network` (no error, no effect — the sandbox is created with default egress), so never rely on it; JS SDK 2.31.0 also lacks the field, but 2.41.0 and later provide it and do send it (which is what `npm install e2b@^2.31.0` on this page resolves to), and the server then rejects the request; calling REST directly always returns `400`.

In restricted mode (`allowPublicTraffic: false`), the public ingress (FC data plane) 403 gate runs before Host rewriting: requests without the `e2b-traffic-access-token` header are rejected first, so the token is required to observe the `maskRequestHost` rewrite.
...
> **Note**: When setting `allowPublicTraffic: false`, you must also set `secure: true`; otherwise the create request is rejected with `400` (`You cannot create a sandbox without public access unless you enable secure envd access via 'secure' flag.`). `secure: true` enables control-plane authentication (`envdAccessToken`, header `X-Access-Token`, carried automatically by the SDK), which is a separate token from the public inbound `trafficAccessToken`.
...
const sandbox = await Sandbox.create({
  ...
  secure: true, // required when allowPublicTraffic is false, otherwise the create request is rejected with 400
  network: {
...
sandbox = Sandbox.create(
    ...
    secure=True,  # required when allow_public_traffic is False, otherwise the create request is rejected with 400
...
- Only the egress-related `allowInternetAccess` / `allowOut` / `denyOut` / `network.rules` can be updated. `allowPublicTraffic` and `maskRequestHost` are **creation-time-locked** parameters and cannot be modified after creation.

## Limitations and Notes

- `egressProxy` is **not yet supported**; passing it over REST returns `400` (Python SDK 2.31.0 has no such field).
