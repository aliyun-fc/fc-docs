...
# The official docs also support the OPENCLAW_GATEWAY_TOKEN environment variable, which is equivalent.
...
# 3. Configure Control UI (required for FC Agent Sandbox public access)
...
**Step 2: Update `OPTS` in the SDK.** Replace the default `OPTS` in [OpenClaw Template](../04.Features/02.Templates/07.Other%20Templates/05.OpenClaw%20Template.md#build-template):
...
After configuring the custom domain and [deploying the Gateway](#deploy-gateway), the examples use the default token-free public access, and the Control UI uses OpenClaw’s Gateway Token for authentication:
...
> **Important**: the Gateway Token in `?token=` is an application credential that ends up in gateway access logs, browser history and the `Referer` header. Never write a Gateway URL containing the token into logs or frontend code, and do not share it publicly; keep its usage minimal and rotate it.

1. Run the script above and note the `Gateway` URL in the output.
2. Open the Gateway URL (the URL already contains `?token=`).

You can first verify page accessibility with `curl`:
...
| Gateway not opening in browser | Bind and use a custom domain first; ensure the URL contains `?token=` |
| Browser triggers a download | A custom domain must be bound; see [FC Agent Sandbox Custom Domains](../04.Features/07.FC%20Extensions/03.Custom%20Domains.md) and [Browser Access (FC Agent Sandbox)](#browser-access) |
