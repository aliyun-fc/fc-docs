...
After configuring the custom domain and [deploying the Gateway](#deploy-gateway), the examples enable public access, and the Control UI uses OpenClaw’s Gateway Token for authentication:
...
| Gateway not opening in browser | Enable `allow_public_traffic` when creating the sandbox; ensure the URL contains `?token=` |
