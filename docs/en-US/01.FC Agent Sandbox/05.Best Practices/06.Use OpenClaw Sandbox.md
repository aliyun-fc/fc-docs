...
print(f"Gateway: {url}")
# X-Access-Token is returned by the SDK after Sandbox.create succeeds and authenticates the FC Agent Sandbox public port proxy
...
|------|------|------|----------|
| Sandbox Access Token | `sandbox._envd_access_token` returned by `Sandbox.create` | FC Agent Sandbox port proxy | Request header `X-Access-Token` |
...
You can verify with `curl` first (must include `X-Access-Token`, otherwise FC Agent Sandbox returns 403):
