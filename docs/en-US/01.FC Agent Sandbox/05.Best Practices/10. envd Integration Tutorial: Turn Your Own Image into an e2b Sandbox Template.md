...
### 2.3 Three necessary conditions
...
- Based on an official `sandbox-*` image: ① and ② are usually already satisfied (**exception**: the standalone `sandbox-browser-tool` image satisfies neither; see [envd-integration.md](envd-integration.md))
- Using **your own image**: ① and ② are likely both missing. You will need to add an nginx/openresty layer yourself; see [Section 6](#6-porting-to-your-own-image)
...
### Step 2: Detect how your image loads process-compose configs

**Do not skip or copy this step across versions.** Three mechanisms have been observed, and **the same image may switch mechanisms between versions**:

```bash
IMG=serverless-registry.cn-hangzhou.cr.aliyuncs.com/functionai/sandbox-all-in-one:v0.10.1

# See how the entrypoint assembles the config list
docker run --rm --entrypoint sh $IMG -c 'grep -n "process-compose" /usr/local/bin/entrypoint.sh'

# Check whether the txt list file is still read (no output = deprecated; appending to it silently does nothing)
docker run --rm --entrypoint sh $IMG -c 'grep -rn process-compose-files.txt /usr/local/bin/ /etc/sandbox/'
```

| Mechanism | Identifying feature | How to modify |
|---|---|---|
| A. Hard-coded bash array | Has `PROCESS_COMPOSE_FILES=(` | `sed` a new element after the array declaration line |
| B. Hard-coded `exec … -f …` | Has `exec process-compose up \` followed by a series of `-f` | `sed` insert a line `-f …envd.yaml \` |
| C. Reads txt list | Code `grep`s/`while read`s the txt | `printf >>` append a line |

all-in-one **v0.10.1 uses mechanism B**. Original snippet:
...
> ⚠️ **In this image `/etc/sandbox/process-compose-files.txt` still exists, but no code reads it any more.**
> all-in-one v0.9.26 used mechanism C; v0.10.1 switched to B, and the old file was not deleted.
> Following the old docs and doing `echo >> …txt` will still let `docker build`, push, and template build succeed,
> **but envd will not be launched at runtime**—the most expensive kind of failure: everything looks green until you create a sandbox.
...
# Mechanism B: insert one -f line into the hard-coded exec command.
# Anchor before the base.yaml line and preserve the line-continuation backslash.
RUN sed -i 's#^\(    -f /etc/sandbox/config/process-compose.base.yaml \\)$#    -f /etc/sandbox/config/process-compose.envd.yaml \\n\1#' /usr/local/bin/entrypoint.sh \
    && [ "$(grep -c 'process-compose.envd.yaml' /usr/local/bin/entrypoint.sh)" = "1" ] \
    && bash -n /usr/local/bin/entrypoint.sh \
    && grep -n 'process-compose\.' /usr/local/bin/entrypoint.sh
```

The three commands after `RUN` are **not decorative; they are build-time self-checks**. Please copy this habit:

| Check | What it catches |
|---|---|
| `grep -c … = 1` | `sed` missed (upstream changed that line) → silent failure; or double insertion |
| `bash -n` | The patch introduced syntax errors (broken line continuations, quotes), causing the container to exit immediately |
| `grep -n 'process-compose\.'` | Prints the final list and order in the build log for human review |

**Why this pedantry is worth it**: you are patching a script inside someone else's image; the anchor point may break on the next upstream upgrade.
Failing `docker build` immediately is much cheaper than debugging after push, template build, and sandbox creation.

> Concrete pitfall: in mechanism A images, if `sed` anchors on the `process-compose.xxx.yaml` string itself,
> it can also match the existence-check loop above the array and insert the envd config path as a shell command.
> With `set -e` in the entrypoint, the container exits immediately. `bash -n` catches this kind of damage right away.

### Step 4: Local validation (the cheapest round)
...
Step ③ is the most valuable: it locally verifies all three conditions from [2.3](#23-three-necessary-conditions) at once.

### Step 5: Push and build the template
...
```python
REGISTRY     = "fc-e2b-dev-registry.cn-beijing.cr.aliyuncs.com"      # for push
REGISTRY_VPC = "fc-e2b-dev-registry-vpc.cn-beijing.cr.aliyuncs.com"  # for from_image
REPO, TAG = "runtime/envd-all-in-one", "v1"
...
### Step 6: Create a sandbox and verify

```python
sbx = Sandbox.create(template="envd-all-in-one-4c8g", timeout=900, api_key=..., api_url=..., domain=...)
try:
...
- [ ] `grep -c` / `bash -n` (and `nginx -t` if applicable) are in `RUN`, so the build fails immediately if the patch breaks
...
| Sandbox is created, but SDK requests return **openresty 404 HTML** | nginx lacks **multi-port routing**; the 49983 request is not forwarded | Add the multi-port config from [Section 6](#6-porting-to-your-own-image) |
...
| The entrypoint `-f` list **does not** contain `envd.yaml` | `sed` missed, or you patched the deprecated txt | Redo mechanism detection from Step 2; add `grep -c … = 1` to `RUN` |
| Container exits immediately with `syntax error near unexpected token` | `sed` broke entrypoint syntax (usually a broken line continuation) | `bash -n` to locate; rewrite `sed` with a different anchor |
...
> That path has not been tested—be sure to verify locally with Step ③ from [Step 4](#step-4-local-validation-the-cheapest-round) before pushing.
