...
The examples enable public access with `network={"allow_public_traffic": True}`, so clients can open `/vnc.html` and connect to `/websockify` directly. The `auth_key` generated by `require_auth` remains the VNC stream password and must be supplied as required by the Desktop SDK.
