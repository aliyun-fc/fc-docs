...
The examples use the default token-free public access, so clients can open `/vnc.html` and connect to `/websockify` directly. The `auth_key` generated by `require_auth` remains the VNC stream password and must be supplied as required by the Desktop SDK.
