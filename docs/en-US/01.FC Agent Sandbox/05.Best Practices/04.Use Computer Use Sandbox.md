...
> **Access**: `allow_public_traffic` defaults to `True`, which means public URLs are reachable **without a traffic token**; when set to `False`, every public request must carry the `e2b-traffic-access-token` header, and query parameters do not work. `secure` defaults to `True` and controls SDK/envd authentication; it is a separate token from the traffic token. The examples omit the default `network` setting and write `secure=True` explicitly. Treat public URLs as access credentials: without application-level authentication, anyone with the URL can fully control the sandbox desktop. Do not share them publicly, and never write them into logs or frontend code. To restrict access, see [Network Access Control](../04.Features/06.Network/05.Network%20Access%20Control.md) and [Shared Responsibility](../06.Security%20Compliance/01.Shared%20Responsibility.md). Restricted API keys in Enforce mode need both `sandbox.network.ingress` (the Python SDK sends `allowPublicTraffic` with any non-empty `network` argument, so it is always set; the JS/TS SDK and plain REST set it only when `allowPublicTraffic` or `maskRequestHost` is given explicitly) and `sandbox.network.egress` (the SDK always sends `allow_internet_access`). In restricted mode, server-side clients can pass `e2b-traffic-access-token` directly in the WebSocket handshake headers; only clients running inside a browser page (such as web noVNC) cannot set custom handshake headers and need a controlled backend proxy, or use `stream.start(require_auth=True)` with `stream.get_url(auth_key=...)`.
...
For Desktop template capabilities and a Python-to-TypeScript API reference, see [Desktop Template](../04.Features/02.Templates/07.Other%20Templates/03.Desktop%20Template.md).
...
- Only one stream can run at a time. Version `v0.0.52` supports whole-desktop streaming but does not support streaming an individual window.

The examples use the default token-free public access, so `/vnc.html` and `/websockify` need no traffic token. However, the default platform domain (`*.e2b.fc.aliyuncs.com`) **triggers a download instead of opening the page**, so [bind a custom domain](../04.Features/07.FC%20Extensions/03.Custom%20Domains.md) before watching it in a browser. The `auth_key` generated by `require_auth` remains the VNC stream password and must be supplied as required by the Desktop SDK.
