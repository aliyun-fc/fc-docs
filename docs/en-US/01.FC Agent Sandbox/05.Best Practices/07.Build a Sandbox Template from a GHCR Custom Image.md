...
The build request passes credentials through the following two HTTP headers, injected via the SDK's `api_headers` parameter:
...
1. Declare the image source with `Template().from_image(<image>)`, and pass the registry credentials via `api_headers`.
