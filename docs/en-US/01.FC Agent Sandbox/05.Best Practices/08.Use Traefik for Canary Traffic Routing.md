...
> In Enforce mode, restricted API keys need both `sandbox.network.ingress` (set by any explicit `network` argument; the examples below set these ingress options) and `sandbox.network.egress` (the SDK always sends `allow_internet_access`) permissions. Treat the traffic token as an access credential and never write it into logs or frontend code. To restrict access, see [Network Access Control](../04.Features/06.Network/05.Network%20Access%20Control.md) and [Shared Responsibility](../06.Security%20Compliance/01.Shared%20Responsibility.md).
...
- The Sandbox needs public network access to download the Traefik binary (the default uses the Serverless OSS global download domain; see "FAQ").
...
# Require traffic-token authentication on public endpoints and keep SDK communication secure.
sbx = Sandbox.create(template="base", timeout=600, secure=True, network={"allow_public_traffic": False}, **conn)
...
> **Note**: The examples use `secure=True` for SDK communication and `network={"allow_public_traffic": False}` to require traffic-token authentication on the public endpoint. External requests send `sbx.traffic_access_token` in the `e2b-traffic-access-token` header.
...
```python
GET_TRAEFIK = r'''
import os, shutil, subprocess, tarfile, time, urllib.request, hashlib
DEST, URL = "/tmp/canary", os.environ["TRAEFIK_TARBALL"]
SHA256 = os.environ.get("TRAEFIK_SHA256", "")
...
# Require SHA-256 verification before extraction to reject tampered or incomplete downloads.
if not SHA256:
    raise SystemExit("TRAEFIK_SHA256 is required")
actual_sha256 = sha256(TAR)
if actual_sha256 != SHA256:
    raise SystemExit("sha256 mismatch: got %s, expected %s" % (actual_sha256, SHA256))
with tarfile.open(TAR) as t:
...
# Traefik binary served from the Serverless OSS global download domain. Override with the TRAEFIK_TARBALL env var
# to use an internal mirror; when overriding, you MUST also provide a matching SHA-256 via TRAEFIK_SHA256.
...
TRAEFIK_TARBALL = os.environ.get(
    "TRAEFIK_TARBALL",
    "https://images.devsapp.cn/e2b/traefik/"
    "v3.7.8/traefik_v3.7.8_linux_amd64.tar.gz",
)
TRAEFIK_SHA256 = os.environ.get(
    "TRAEFIK_SHA256",
...
res = sbx.commands.run(
    ...
    envs={"TRAEFIK_TARBALL": TRAEFIK_TARBALL, "TRAEFIK_SHA256": TRAEFIK_SHA256},
    timeout=600,
)
...
# Public endpoints require traffic-token authentication; SDK communication remains secure=True.
...
The default download URL points to the Serverless OSS global download domain, which is reachable worldwide and does not rely on GitHub. Suggestions:

- The download is integrity-checked against the built-in SHA-256 after completing. If you override the URL via the `TRAEFIK_TARBALL` environment variable (for example, to use your own object storage / internal mirror), you **must** also provide a matching SHA-256 via `TRAEFIK_SHA256` so the integrity check still passes.
- **Recommended for production**: bake the Traefik binary into a **custom Sandbox template image** so no download is needed at runtime (see [Build a Sandbox Template from a GHCR Custom Image](./07.Build%20a%20Sandbox%20Template%20from%20a%20GHCR%20Custom%20Image.md)).
...
Set `secure=True` and `network={"allow_public_traffic": False}`. Send `sbx.traffic_access_token` in the `e2b-traffic-access-token` header when accessing dynamic ports. Never write access tokens to logs or frontend code.
...
Combine the steps above into a single runnable script `canary_demo.py`. Compared with the demo snippets in the procedure, this script adds production hardening: it creates the sandbox with `secure=True` and `network={"allow_public_traffic": False}` and automatically attaches `e2b-traffic-access-token`, adds port-readiness polling, asserts the ratio for each weight tier, and cleans up processes and the Sandbox in a `finally` block. `E2B_API_KEY`, `E2B_DOMAIN`, and `E2B_API_URL` are all read from environment variables.
...
def auth(sbx: Sandbox) -> dict:
    """Sandboxes with traffic tokens enabled need e2b-traffic-access-token to access dynamic ports; traffic_access_token is None when not enabled."""
    token = sbx.traffic_access_token
...
def check_ratio(sbx: Sandbox, w1: int, w2: int) -> None:
    ...
    # First reject errors/unknown responses, then validate the weighted ratio.
    assert hits["v1"] + hits["v2"] == N_REQUESTS, f"unexpected hits: {dict(hits)}"
    exp_v1 = w1 / (w1 + w2) * 100
...
def main() -> int:
    ...
    sbx = Sandbox.create(template=TEMPLATE, timeout=SANDBOX_TIMEOUT, secure=True, network={"allow_public_traffic": False}, **conn_opts())
...
- [Network Access Control](../04.Features/06.Network/05.Network%20Access%20Control.md)
- [Shared Responsibility](../06.Security%20Compliance/01.Shared%20Responsibility.md)
