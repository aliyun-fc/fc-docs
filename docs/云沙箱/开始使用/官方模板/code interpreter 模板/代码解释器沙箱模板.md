...
Code Interpreter 模板提供安全隔离的代码执行沙箱环境，支持在云端安全地执行 Python、JavaScript 等语言代码，并提供文件管理、终端命令执行、上下文管理等完整的数据面 API。
...
| 安全隔离 | 基于 MicroVM 级别隔离，每个沙箱实例拥有独立的文件系统和进程空间 |
...
1. **创建 Code Interpreter 沙箱模板**：通过控制台或 OpenAPI 创建模板。
...
- [会话介绍](../../功能说明/会话介绍.md)
- [Browser Tool 模板](../browser%20模板/browser-tool-模板.md)（仅需浏览器自动化能力时选择）
