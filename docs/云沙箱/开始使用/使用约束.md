...
本文档示例中的安装命令代表对应示例的验证版本。不同示例可能来自不同验证批次，接入时需要同时关注安装命令和[E2B 兼容说明](./E2B%20兼容说明.md)中的能力边界。
...
E2B CLI 还需要读取 `E2B_ACCESS_TOKEN`。接入云沙箱时，其值与 `E2B_API_KEY` 相同：

```bash
export E2B_ACCESS_TOKEN=$E2B_API_KEY
```

API Key 应在函数计算控制台创建和管理，不通过 E2B SDK 管理。不要在日志、截图、工单或前端页面中暴露完整 API Key。
...
更多模板构建要求参见[自定义模板](../功能说明/Templates/自定义模板.md)。
...
Sandbox、Commands、Filesystem、Code Interpreter、Template 和 CLI 的接口边界集中在[E2B 兼容说明](./E2B%20兼容说明.md)中维护。接入前应确认业务依赖的接口已在支持范围内；未兼容能力不应作为云沙箱接入主路径。
