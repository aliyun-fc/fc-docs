...
自定义模板允许您使用自己的容器镜像创建沙箱环境，预装业务所需的依赖、工具和证书，构建标准化的专属沙箱。自定义模板通过 `container_configuration` 指定容器镜像地址和启动命令，不局限于预置的 Code Interpreter、Browser Tool、All-in-One 三种镜像。
